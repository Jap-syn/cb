...
$this->logger->info('hourlyUpdateClaimPrint.php start');
...
$this->logger->info('hourlyUpdateClaimPrint.php end');
...
FROM
	T_Site s
	INNER JOIN T_Enterprise e ON s.EnterpriseId = e.EnterpriseId 
WHERE
	(e.UpdateDate >= :before) OR s.SiteId in (SELECT SiteId FROM T_SiteSbpsPayment WHERE UseStartDate < :now AND UseStartDate >= :before)
ORDER BY
	s.EnterpriseId,
	SiteId
EOQ;
        $sites = $this->dbAdapter->query($sql)->execute(array(':before'=>date("Y-m-d H:i:00",strtotime("-1 hour")), ':now'=>date('Y-m-d H:i:00')));

        foreach ($sites as $site) {
            $mdlCode = new TableCode($this->dbAdapter);
            $is_saved = $mdlCode->find2(214, $site['EnterpriseId'])->count();
            if ($is_saved != 0) {
                continue;
            }
