...
$this->logger->info('createAccoutTransferMhf.php start');
            ...
            // ファイル名生成
            $fileName = ('MHFFURIKAE' . date('YmdHis').'.txt');
...
$this->logger->info('createAccoutTransferMhf.php end');
            $exitCode = 0;
...
//            }
            ...
            $sql .= " ,      e.MhfCreditTransferDisplayName ";
            ...
            $ri = $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[3]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[3]['SpanTo'].' 23:59:59'));
            if (!($ri->count() > 0)) {
            ...
            $h_typeCode         = '91';
            $h_codeDivision     = '1';
            $h_multiCode        = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSConsignorCodeMHF');
            $h_consignorCode    = '0000';
            $h_consignorName    = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSConsignorNameMHF');
//            $h_transderDate     = date('m05', strtotime($today . ' +1 month')); // 処理月の次月5日
            $transderDate = $lgc->getTransderDate(3);
...
//            $h_accountNumber    = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSAccountNumberMufj');
            ...
            foreach ($ri as $row) {
                ...
                $d_branchName       = mb_substr(mb_convert_kana($row['MhfCreditTransferDisplayName'], 'ask').'               ', 0, 15);
                $d_dummy            = '0000';
                $d_depositType      = $row['FfAccountClass'];
                $d_accountNumber    = $row['FfAccountNumber'];
                $d_depositorName    = mb_substr($row['FfAccountName'].'                              ', 0, 30);
                $d_claimAmount      = sprintf('%010d', $row['ClaimAmount']);
                $d_newCode          = '1'; // [0:2回目以降の請求の場合]に固定
                $work = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSConsignorCodeMHF');
                $d_consignorCode    = substr($work, 0, 6);
                $d_entCustSeq       = sprintf('%014d', $row['EntCustSeq']);
                $d_resultCode       = '0';
                $d_reserve          = '        ';
