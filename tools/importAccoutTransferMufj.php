...
class Application extends BaseApplicationAbstract {
    ...
    public function run() {
        ...
        try {
            ...
            $row = $this->dbAdapter->query(" SELECT Seq FROM T_ImportedAccountTransferFile WHERE FileName = :FileName AND CreditTransferFlg=2 ")->execute(array(':FileName' => $_SERVER['argv'][1]))->current();
            if ($row) {
                // (更新 ※インポートエラーからの再処理)
                $seq = $row['Seq'];
                $upddata = array (
            ...
            $seqcsv = $seq;
            $row = $this->dbAdapter->query(" SELECT ReceiptResult FROM T_ImportedAccountTransferFile WHERE Seq = :Seq AND CreditTransferFlg=2 ")->execute(array(':Seq' => $seq))->current();
            ...
            if($data['OrderId'] !== ""){
                ...
                $addition = $mdlc->find2(219, $data['ResCode'])->current()['Class2'];
        ...
        try {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                if ($recordCode == 1) {
                    ...
                    $sql .= " WHERE  1 = 1 ";
                    $sql .= " AND    e.CreditTransferFlg IN (1, 2, 3) ";
...
//                    $sql .= " AND    ch.CreditTransferRequestStatus = 2 ";
                    ...
                    $row = $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[2]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[2]['SpanTo'].' 23:59:59', ':EntCustSeq' => (int)$d_entCustSeq, ':ClaimAmount' => (int)$d_claimAmount))->current();
                    ...
                    if ($d_resultCode != '0') {
                        ...
                        $checkRow= $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[2]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[2]['SpanTo'].' 23:59:59', ':EntCustSeq' => (int)$d_entCustSeq, ':ClaimAmount' => (int)$d_claimAmount))->current();
                        //
                        if (!empty($checkRow)) {
                            //
                            $infodata2[] = array(
                    ...
                    $prm = array(
                        ...
                        ':pi_receipt_agent_id' => 6,
                        ':pi_deposit_date'     => $lgc->getTransderCommitDate($receiptDate, 2),
                        ':pi_user_id'          => $userId,
