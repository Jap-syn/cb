...
//             // 会計帳票/CSV作成処理
//             // (業務日付（バッチの実行時に利用する想定）)
//             $accountingMonth = $this->dbAdapter->query(" SELECT PropValue FROM T_SystemProperty WHERE Module = '[DEFAULT]' AND Category = 'systeminfo' AND Name = 'AccountingMonth' "
...
$this->logger->err($e->getTraceAsString());
    ...
    protected function _saveATReportFileMonthly($today, $filename)
    {
        ...
        $cnt = (int)$this->dbAdapter->query(" SELECT COUNT(1) AS cnt FROM AT_ReportFileMonthly WHERE CreateDate = :CreateDate "
                )->execute(array(':CreateDate' => $today))->current()['cnt'];
        if ($cnt != 0) {
            $this->dbAdapter->query(" DELETE FROM AT_ReportFileMonthly WHERE CreateDate = :CreateDate ")->execute(array(':CreateDate' => $today));
        }
        // 登録
        // ユーザーID(バッチユーザー)の取得
