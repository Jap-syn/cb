...
$this->dbAdapter->getDriver()->getConnection()->beginTransaction();
        ...
        foreach($arrOseq as $key => $value) {
            ...
            $this->logger->info('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'complete!!');
        ...
        foreach($arrOrderId as $key => $value) {
            ...
            if (!$row) {
                // データが取得出来ない場合
                $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'OrderId Is Not Found');
                continue;
            }
...
$this->dbAdapter->getDriver()->getConnection()->commit();
    ...
    public function isReceiptCancel($key, $value, $cntDouble) {
        ...
        if (!$row) {
            // データが取得出来ない場合
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'OrderSeq Is Not Found');
            return false;
        }

        // キャンセルされていないこと
        if ($mdlo->isCanceled($key)) {
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'Is Cancel');
            return false;
        }

        // 一部入金もしくは、入金クローズではない
        if ( !(($row['DataStatus'] == 61) || ($row['DataStatus'] == 91 && $row['CloseReason'] == 1)) ) {
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'DataStatus Is InValid[DataStatus=' . $row['DataStatus'] . ',CloseReason=' . $row['CloseReason'] . ']');
            return false;
        }
        ...
        if ($cnt > 0) {
            // 返金済み
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'Is Repayment Input');
            return false;
        }
        ...
        if ($cnt > 0) {
            // 雑損失または雑収入の入力あり
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'Is Sundry Input');
            return false;
        }
        ...
        // 入金件数が3件ではない
        if ( $cnt <> $cntDouble ) {
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'Not ' . $cntDouble . 'Count');
            return false;
        }

        // 入金額が3倍ではない
        if ( !($sum == ($value * $cntDouble)) ) {
            $this->logger->alert('[DoubleReceipt]' . "\t" . $key . "\t" . $value . "\t" . 'Not Summary Amount');
            return false;
        }
