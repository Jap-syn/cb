...
$this->logger->err($e->getTraceAsString());
    ...
    public function _exec()
    {
        ...
        // 対象データ取得
        // データステータス：41、51、61 かつ 注文マイページが作成されていないものを抽出
        $sql = "";
        $sql .= " SELECT  o.OrderSeq ";
        $sql .= "     ,   ( SELECT MAX(LimitDate) FROM T_ClaimHistory WHERE OrderSeq = o.OrderSeq AND ValidFlg = 1 AND ClaimPattern = 1) AS F_LimitDate ";
        $sql .= "     ,   IFNULL(o.OemId, 0) AS OemId ";
        $sql .= " FROM    T_Order o ";
        $sql .= " WHERE   o.DataStatus IN (41, 51, 61) ";
        $sql .= " AND     0 = (SELECT COUNT(1) FROM T_MypageOrder WHERE OrderSeq = o.OrderSeq) ";
        $sql .= " AND     ( SELECT COUNT(1) FROM T_ClaimHistory WHERE OrderSeq = o.OrderSeq AND ValidFlg = 1) > 0 ";
