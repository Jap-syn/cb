...
$this->logger->info('createAccoutTransfer.php start');
...
$this->logger->info('createAccoutTransfer.php end');
            $exitCode = 0;
...
//            }
            ...
            $ri = $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[1]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[1]['SpanTo'].' 23:59:59'));
            if (!($ri->count() > 0)) {
            ...
            $h_codeDivision     = '1';
            $h_multiCode        = '00';
            $h_consignorCode    = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSConsignorCode');
            $h_consignorName    = $mdlsp->getValue(\models\Table\TableSystemProperty::DEFAULT_MODULE, 'systeminfo', 'CATSConsignorName');
//            $h_transderDate     = date('m05', strtotime($today . ' +1 month')); // 処理月の次月5日
            $transderDate = $lgc->getTransderDate(1);
