...
$this->logger->warn('The argument 1 is not a number. argv=' . $_SERVER['argv'][1]);
                exit(0);
            }
            $creditThreadNo = (int)$_SERVER['argv'][1];

            if (!is_numeric($_SERVER['argv'][2])) {
$this->logger->warn('The argument 2 is not a number. argv=' . $_SERVER['argv'][2]);
                exit(0);
            }
            $cjBatchSeq = (int)$_SERVER['argv'][2];
echo "Target Batch Seq [".$cjBatchSeq."]\r\n";
...
//            if($lockId > 0) {
                ...
                try {
                    ...
                    $this->dbAdapter->query('UPDATE T_CjBatchControl SET Status=2, Status2Date=NOW(), UpdateDate=NOW() WHERE CjBatchSeq = :CjBatchSeq AND Status=1')->execute(array(':CjBatchSeq'=>$cjBatchSeq));
                    // 多重起動制御（楽観ロックをやりたかったけど、Zend2がよくわからないので・・・
                    $cnt = $this->dbAdapter->query('SELECT COUNT(*) AS CNT FROM T_CjBatchControl WHERE CjBatchSeq = :CjBatchSeq AND Status=2')->execute(array(':CjBatchSeq'=>$cjBatchSeq))->current()['CNT'];
echo "Status Update Result [".$cnt."]\r\n";
                    if ($cnt == 0) {
echo "Skip Process.\r\n";
                        $exitCode = 100; // 正常終了
                    } else {
...
//                        $judge_result = $logic->doJudgementForBatch($creditThreadNo);
                        $judge_result = $logic->doJudgementForAwsBatch($creditThreadNo, $cjBatchSeq);
...
//                    }
                    ...
                    $exitCode = 0; // 正常終了
                    $this->dbAdapter->query('UPDATE T_CjBatchControl SET Status=9, UpdateDate=NOW() WHERE CjBatchSeq = :CjBatchSeq')->execute(array(':CjBatchSeq'=>$cjBatchSeq));
                    }
