...
class Application extends BaseApplicationAbstract {
    ...
    public function run() {
        ...
        try {
            ...
            $this->logger->info('importAccoutTransferMhf.php start');
            ...
            $row = $this->dbAdapter->query(" SELECT Seq FROM T_ImportedAccountTransferFile WHERE FileName = :FileName AND CreditTransferFlg=3 ")->execute(array(':FileName' => $_SERVER['argv'][1]))->current();
            if ($row) {
                // (更新 ※インポートエラーからの再処理)
                $seq = $row['Seq'];
                $upddata = array (
            ...
            $seqcsv = $seq;
            $row = $this->dbAdapter->query(" SELECT ReceiptResult FROM T_ImportedAccountTransferFile WHERE Seq = :Seq AND CreditTransferFlg=3 ")->execute(array(':Seq' => $seq))->current();
            ...
            if($data['OrderId'] !== ""){
                ...
                $addition = $mdlc->find2(219, $data['ResCode'])->current()['Class3'];
            ...
            $this->logger->info('importAccoutTransferMhf.php end');
            $exitCode = 0;
        ...
        try {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                if ($recordCode == 1) {
                    ...
                    $sql .= " WHERE  1 = 1 ";
                    $sql .= " AND    e.CreditTransferFlg IN (3) ";
...
//                    $sql .= " AND    ch.CreditTransferRequestStatus = 2 ";
                    ...
                    $row = $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[3]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[3]['SpanTo'].' 23:59:59', ':EntCustSeq' => (int)$d_entCustSeq, ':ClaimAmount' => (int)$d_claimAmount))->current();
                    ...
                    if ($d_resultCode != '0') {
                        ...
                        $checkRow= $this->dbAdapter->query($sql)->execute(array(':fr_date' => $tarm[3]['SpanFrom'].' 00:00:00', ':to_date' => $tarm[3]['SpanTo'].' 23:59:59', ':EntCustSeq' => (int)$d_entCustSeq, ':ClaimAmount' => (int)$d_claimAmount))->current();
                        //
                        if (!empty($checkRow)) {
                            //
                            $infodata2[] = array(
                    ...
                    $prm = array(
                        ...
                        ':pi_receipt_agent_id' => 5,
                        ':pi_deposit_date'     => $lgc->getTransderCommitDate($receiptDate, 3),
                        ':pi_user_id'          => $userId,
