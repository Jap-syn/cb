...
$this->logger->err($e->getMessage());
    ...
    protected function _sendReceiptConfirmMail()
    {
        ...
        $ri = $this->dbAdapter->query(" SELECT ReceiptSeq FROM T_ReceiptControl rc INNER JOIN T_Order o ON (o.OrderSeq = rc.OrderSeq) INNER JOIN T_ClaimControl c ON ( o.OrderSeq = c.OrderSeq ) WHERE (rc.MailFlg = 0 AND rc.MailRetryCount < 5) AND rc.ValidFlg = 1 AND o.DataStatus = 91 AND o.CloseReason = 1 AND ((c.F_CreditTransferDate IS NULL) OR (c.F_CreditTransferDate IS NOT NULL AND rc.ReceiptClass != 13)) ")->execute(null);
        ...
        foreach($ri as $row) {
            ...
            try {
                ...
                $mail->SendRcptConfirmMail($row['ReceiptSeq'], $userId);
