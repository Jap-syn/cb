DROP PROCEDURE IF EXISTS P_Treas_Excess_List_Month;

DELIMITER $$

CREATE PROCEDURE P_Treas_Excess_List_Month ( IN pi_user_id INT )
proc:
/******************************************************************************
 *
 * ﾌﾟﾛｼｰｼﾞｬ名       ：  P_Treas_Excess_List_Month
 *
 * 概要             ：  過剰金一覧（月次）
 *
...
BEGIN
    ...
    INTO    AT_Excess_List( DailyMonthlyFlg                                                        -- 日次･月次区分
                    ...
                    FROM    T_ClaimControl cc
                            ...
                            INNER JOIN AW_Receipt1 rc1 ON (rc1.OrderSeq = o.OrderSeq)
                            INNER JOIN T_ReceiptControl rc ON (rc.ReceiptSeq = rc1.ReceiptSeq)
                    ...
                    AND     DATE(rc.ReceiptProcessDate) <= v_BusinessDate
                    AND     cc.ValidFlg = 1
                    AND     o.ValidFlg  = 1
                    AND     (can.ApprovalDate IS NULL OR can.ApprovalDate >= (v_AccountingMonth + INTERVAL 1 MONTH))
                    ;
    ...
    INTO    AT_Excess_List( DailyMonthlyFlg                                                        -- 日次･月次区分
                    ...
                    FROM    T_ClaimControl cc
                            ...
                            INNER JOIN M_Code c6 ON (c6.CodeId = 160 AND c6.KeyCode = IFNULL(o.OemId, 0))
                            LEFT OUTER JOIN T_Cancel can ON (can.OrderSeq = o.OrderSeq)
                    WHERE   (cc.BalanceClaimAmount > 0 AND rc1.ReceiptAmount > 0)
    ...
    INTO    AT_Excess_List( DailyMonthlyFlg                                                        -- 日次･月次区分
                    ...
                    FROM    T_ClaimControl cc
                            ...
                            INNER JOIN M_Code c5 ON (c5.CodeId = 159 AND c5.KeyCode = IFNULL(o.OutOfAmends, 0))
                            INNER JOIN M_Code c6 ON (c6.CodeId = 160 AND c6.KeyCode = IFNULL(o.OemId, 0))
                    WHERE   (cc.BalanceClaimAmount > 0 OR cc.BalanceClaimAmount + rc1.ReceiptAmount > 0)
                    AND     DATE_FORMAT(pc.ExecScheduleDate, '%Y-%m') > DATE_FORMAT(v_AccountingMonth, '%Y-%m')       -- 振込日（立替予定日）が未来日のﾃﾞｰﾀが対象
                    AND     DATE(rc.ReceiptProcessDate) <= v_BusinessDate
                    AND     cc.ValidFlg = 1
                    AND     o.ValidFlg  = 1
                    AND     (can.ApprovalDate < (v_AccountingMonth + INTERVAL 1 MONTH))
    ...
    INTO    AT_Excess_List( DailyMonthlyFlg                                                        -- 日次･月次区分
                    ...
                    WHERE   (cc.BalanceClaimAmount > 0 OR cc.BalanceClaimAmount + rc1.ReceiptAmount > 0)
                    AND     DATE_FORMAT(oc.SettlePlanDate, '%Y-%m') > DATE_FORMAT(v_AccountingMonth, '%Y-%m')     -- 振込日（精算日）が未来日のﾃﾞｰﾀが対象
                    AND     DATE(rc.ReceiptProcessDate) <= v_BusinessDate
                    AND     cc.ValidFlg = 1
                    AND     o.ValidFlg  = 1
                    AND     (can.ApprovalDate < (v_AccountingMonth + INTERVAL 1 MONTH))
