DROP VIEW IF EXISTS `V_WaitForFirstClaim`;

CREATE VIEW `V_WaitForFirstClaim`
AS
...
-- 	,`VOC`.`ClaimFee` AS `ClaimFee`
	...
	,`VOC`.`CombinedClaimTargetStatus` AS `CombinedClaimTargetStatus`
	,(
-- 		SELECT count(0) AS `COUNT(*) `
		SELECT count(0)
		FROM `T_ClaimHistory`
		WHERE (
				(`T_ClaimHistory`.`OrderSeq` = `VOC`.`OrderSeq`)
				AND (`T_ClaimHistory`.`PrintedFlg` = 0)
				)
		) AS `BookedCount`
