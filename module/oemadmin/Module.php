...
class Module implements LocatorRegisteredInterface
{
    ...
    public function boforeDispatch(MvcEvent $event){
        ...
        if (!$app->authManagerAdmin->isAuthenticated()) {
            ...
            // ユーザーIDの取得
            $userId = $app->authManagerAdmin->getUserInfo()->UserId;
            /// ユーザー名の取得
            ...
            $passData = $tblPass->findnew(4, $LoginId)->current();

            if(!$app->authManagerAdmin->getAlternativeUserInfo() && (empty($passData) || $passData['PasswdLimitDay'] < date('Y-m-d')) && !in_array ( $requestedResourse, $whiteList2 )){
                $event->getRouteMatch()->setParam('controller', __NAMESPACE__ . '\Controller\Operator')->setParam('action', 'changepw');
            }
        }
    ...
    public function dispatchError(MvcEvent $event){
        ...
        $variables = array(
                ...
                'styleSheets' => array(0 => Application::getInstance()->getOemCss()),
                'javaScripts' => array(),
