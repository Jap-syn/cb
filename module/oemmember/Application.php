...
class Application extends BaseApplicationOem {
    protected $_application_id = 'OemMemberApplication';

    // アプリケーションディレクトリにアクセスするURLのサブディレクトリ名
    protected $_subDirName = 'member';
    ...
    public function run() {
        ...
        // [oemmember\config\config.ini]の値の取得
        ...
        if(isset($this->appGlobalConfig['page_title_prefix'])) {
            $oemInfo = $this->getCurrentOemData();
            $this->appGlobalConfig['page_title_prefix'] = sprintf('%s%s', nvl($oemInfo['OemNameKj'], ''), $this->appGlobalConfig['page_title_prefix']);
        }
...
$this->logger->debug(sprintf('[Application::checkAccount] EnterpriseId = %s, account oem id = %s', $rowObj->EnterpriseId, $rowObj->OemId));
        // アカウント無効ならNG
        if(!$rowObj->ValidFlg) {
$this->logger->debug('[Application::checkAccount] account innactive');
            return false;
        }

        // 非OEM配下の事業者アカウントはNG
        if(!$rowObj->OemId) {
$this->logger->debug('[Application::checkAccount] non oem enterprise');
            return false;
        }
...
$this->logger->debug(sprintf('[Application::checkAccount] oem data not found or other oem data found, oem id = %s', $dbRow ? $dbRow->OemId : '(null)'));
            return false;
        }

        // 代理アカウントデータがあり、且つ無効な場合はNG
        if (!is_null($altRowObj) && !$altRowObj->ValidFlg) {
$this->logger->debug('[Application::checkAccount] alternative account innactive');
            return false;
        }
...
$this->logger->debug('[Application::checkAccount] alternative login (singlecolon)');
                if($altRowObj->OemId != $rowObj->OemId) {
$this->logger->debug(sprintf('[Application::checkAccount] not parent oem, alt oem id = %s', $altRowObj->OemId));
                    return false;
                }
            } else {
$this->logger->debug('[Application::checkAccount] alternative login (doublecolon)');
            }
        }

        // ここまで到達したらOK
$this->logger->debug('[Application::checkAccount] additional authentication success');
        return true;
    }
    ...
    public function getOemCss($oemId) {
        ...
        return '../../oemmember/css/' . $css['Class2'];
    }
    ...
    protected function initAttemptJudgeLogic() {
        if($this->_attemptJudge == null) {
            $this->_attemptJudge = new CoralAuthAttemptJudge($this->dbAdapter, CoralAuthAttemptJudge::APP_OEMMEMBER);
            $this->_attemptJudge
    ...
    protected function checkPasswordValidity($loginId) {
        ...
        $appName = LogicAccountValidity::APPNAME_OEMMEMBER;
...
EOQ;
        ...
        // --------------------------
        // OEMの場合は、OEMマスタのヘルプURLを指定する
        // --------------------------
        if (isset($menuLinks[6])) {
            $oemId = $this->authManager->getUserInfo()->OemId;
            $oemInfo = $this->getOemInfo($oemId)->current();
            if(!is_null($oemId) && $oemId != 0) {
                $menuLinks[6]['href'] = $oemInfo['HelpUrl'];
                if(strlen($oemInfo['HelpUrl']) == 0 || is_null($oemInfo['HelpUrl'])){
                    unset($menuLinks[6]);
                }
            }
        }
