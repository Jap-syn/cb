...
class AccountController extends CoralControllerAction {
    ...
    protected function _init() {
        ...
        $this->addStyleSheet( '../../oemmember/css/index.css' );
        $this->addStyleSheet( '../../oemmember/css/tab_support.css' );
        $this->addStyleSheet( '../../oemmember/css/account.css' );
        ...
        $this->_oemInfo = $this->app->getOemInfo($userInfo->OemId)->current();

        // 変更可能項目の検証ルールリスト
        $this->_validators = array();
        $edit_limitation = true;
        if(!$this->_oemInfo['EntAccountEditLimitation']) {
            // 編集項目制限フラグがONでない場合のみ有効な検証ルールを構築
            $edit_limitation = false;
        ...
        $this->view->assign('oemInfo', $this->_oemInfo);
        $this->view->assign('cssName', "account");
        $this->view->assign('editLimitation', $edit_limitation);
    ...
    public function indexAction() {
        ...
        $newPassHashed = $authUtil->generatePasswordHash($this->app->authManager->getUserInfo()->LoginId, trim($params['NewPassword']));
        ...
        foreach ($siteListAry as $key => $siteId) {
            if ($siteId['ReceiptAgentId'] == 2 and $siteId['MufjBarcodeUsedFlg'] == 0) {
...
//             'CreditNgDispDays', 'JournalRegistDispClass', 'DispOrder1', 'DispOrder2', 'DispOrder3'
//         ) as $key ) {
        foreach(array_keys($this->_validators) as $key) {
            $entRow[$key] = $postData[$key];
        }
        // 事業者情報編集の制限によらないので常に更新する
        foreach( array(
    ...
    protected function validate($data = array())
    {
        ...
        $lastPass = ResultInterfaceToArray($this->app->dbAdapter->query($sql)->execute(array(':Category' => 3, ':LoginId' => $this->app->authManager->getUserInfo()->LoginId)));
