...
class RegistController extends CoralControllerAction {
    ...
    */
    protected function _init() {
        ...
        {
            $this->addStyleSheet( '../../oemmypage/css_sp/mypage_regist.css' );
        }
        else
        {
            $this->addStyleSheet( '../../oemmypage/css/mypage_regist.css' );
        }
    ...
    */
    public function preregistAction() {
        ...
        if (!isset($params['orderseq'])) {
            ...
            $oemId = $mdlo->find( $params['orderseq'] )->current()['OemId'];
    ...
    public function registAction() {
        ...
        $mypageRegist = $this->app->dbAdapter->query( $sql )->execute( array( ':UrlParameter' => $param['token'], ':OemId' => $this->oemId ) )->current();
        ...
        $mypageRegist['MembershipAgreement3'] = $sysPro->getValue('[DEFAULT]', 'systeminfo', 'MembershipAgreement3');
    ...
    public function confirmAction() {
        ...
        $data['MembershipAgreement3'] = $sysPro->getValue('[DEFAULT]', 'systeminfo', 'MembershipAgreement3');
        $mdlsp = new TableSystemProperty($this->app->dbAdapter);
    ...
    public function saveAction() {
        ...
        if( $this->app->dbAdapter->query( $sql )->execute( $prm )->current() ) {
            ...
            // ハッシュ済みのログインパスワードを設定
            $data['LoginId'] = $data['OemId']. "@". $data['MailAddress'];
    ...
    private function validate( $data = array() ) {
        ...
        $mdlsp = new TableSystemProperty($this->app->dbAdapter);
        $propValue = $mdlsp->getValue("[DEFAULT]", "systeminfo", "PasswdCount");
        if( !isset( $errors[$key] ) && !( preg_match( '/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[ -\/:-@\[-`\{-\~])[0-9a-zA-Z -\/:-@\[-`\{-\~]{' . $propValue . ',}$/', $data[$key] ) ) ) {
            $errors[$key] = 'パスワードは英大文字、英小文字、数字、記号のすべてを含む' . $propValue . '文字以上でご入力ください。';
        }
