...
class LoginController extends CoralControllerAction {
    ...
    */
    protected function _init() {
        ...
        {
            $this->addStyleSheet( '../../oemmypage/css_sp/mypage_login.css' );
        }
        else
        {
            $this->addStyleSheet( '../../oemmypage/css/mypage_login.css' );
        }
        // 認証マネージャとFlashMessengerをメンバに設定
        $this->authManager = Application::getInstance()->authManager;
        $this->messenger = $this->flashMessenger();

        $oemTable = new MypageViewOem($this->app->dbAdapter);
        $dbRow = $oemTable->findByAccessId($this->app->getOemAccessId())->current();
        $this->oemId = $dbRow['OemId'];

        $this->cookieName = $this->oemId. "@". "autologin";
    }
    ...
    */
    public function loginAction() {
        ...
        $this->view->assign( 'oemid', $this->oemId );
    ...
    */
    public function authAction() {
        ...
        if (! empty($_COOKIE[$this->cookieName])) {
            ...
            if (! empty($param['autologin'])) {
                ...
                $this->app->dbAdapter->query($sql)->execute(array(':MailAddress' => $param['login_id'], ':OemId' => $this->oemId));
        ...
        $PassHist = ResultInterfaceToArray($mdlph->findnew(6, $userInfo->LoginId));

        $passAlert = $mdlsp->getValue('[DEFAULT]', 'systeminfo', 'PasswdLimitAlertDay');

        if(!empty($PassHist) && (($PassHist[0]['PasswdLimitDay'] >= date('Y-m-d') && $PassHist[0]['PasswdLimitDay'] <= date('Y-m-d', strtotime($passAlert ." day")))) || $PassHist[0]['PasswdLimitDay'] < date('Y-m-d')){
            $target = 'edit/chgpw';
        }
        // ===========================================================================
    ...
    public function setupAutoLogin($mailAddress)
    {
        ...
        $prm = array(':MailAddress' => $mailAddress, ':AutoLoginKey' => $autoLoginKey, ':BrowserInfo' => $_SERVER['HTTP_USER_AGENT'], ':RegistDate' => date('Y-m-d H:i:s'), ':UpdateDate' => date('Y-m-d H:i:s'), ':OemId' => $this->oemId);
        $this->app->dbAdapter->query($sql)->execute($prm);
    ...
    */
    ...
    public function reissuecompleteAction() {
        ...
        // マイページパスワード再発行メールの送信
        $baseUrl = $this->app->dbAdapter->query(" SELECT KeyContent FROM MV_Code WHERE CodeId = 107 AND KeyCode = :OemId ")->execute(array(':OemId' => $this->oemId))->current()['KeyContent'];
        $lcmail = new LogicMail($this->app->dbAdapter, $this->app->smtpServer);
        $lcmail->SendPasswordResetMail(array('OemId' => $this->oemId, 'MailAddress' => $row['MailAddress'], 'UrlParameter' => $accessKey), $baseUrl);

        // よくある質問リンク
        $linkfaq = $this->app->dbAdapter->query(" SELECT KeyContent FROM MV_Code WHERE CodeId = 103 AND KeyCode = :OemId ")->execute(array(':OemId' => $this->oemId))->current()['KeyContent'];
    ...
    public function resetcompleteAction() {
        ...
        $newPassHashed = $authUtil->generatePasswordHash( $this->oemId . "@". $row['MailAddress'], $formdata['pwd'] );
        ...
        foreach ($LastPass as $pass)
        {
            ...
            {
                $errors[] = '過去' .$passTimeLimit. '回に使用したパスワードは利用できません。';
            }
        }
     ...
     * IDを忘れた方ページ
     */
    public function forgetidAction() {
        $this->setPageTitle( 'IDを忘れた方' );

        $contact = '';
        if      ($this->oemId == 1) { $contact = 'TEL: 0570-05-1210（平日 10:00～18:00）'; }
        else if ($this->oemId == 2) { $contact = 'TEL: 0120-667-690（10:00 ～ 18:00）'; }
        else if ($this->oemId == 3) { $contact = 'TEL: 03-4326-3610（9:00～18:00）'; }
        else if ($this->oemId == 4) { $contact = ''; }

        $note = sprintf("IDは登録時に設定したメールアドレスです。<br />不明な場合は、弊社サポートセンターまで、ご連絡ください。<br /> %s", $contact);
