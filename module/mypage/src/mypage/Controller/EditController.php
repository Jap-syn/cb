...
class EditController extends CoralControllerAction {
    ...
    */
    public function passchgconfirmAction() {
        ...
        $loginPasswd = $authUtil->generatePasswordHash( "0". "@". $this->userInfo->MailAddress, $data['LoginPasswd'] );

        $newLoginPasswd = $authUtil->generatePasswordHash( "0". "@". $data['MailAddress'], $data['NewLoginPasswd'] );

        $mdlsp = new TableSystemProperty($this->app->dbAdapter);
        // パスワード文字数(桁)の取得
        $passwdCount = $mdlsp->getValue( '[DEFAULT]', 'systeminfo', 'PasswdCount' );
        $data['PasswdCount'] = $passwdCount;
        // 過去パスワード使用回数(回)の取得
        ...
        {
            if ($newLoginPasswd == $pass['LoginPasswd'])
            {
                $error[] = '過去' . $usePasswdTimes . '回に使用したパスワードは利用できません。';
            }
        }
    ...
    */
    private function validate( $data = array() ) {
        ...
        // PostalCode: 郵便番号
        $key = 'PostalCode';
        $cralv = new CoralValidate();
        if( !isset( $errors[$key] ) && strlen( $data[$key] ) == 0 ) {
            $errors[$key] = "郵便番号が未入力です。";
        }
        if( !isset( $errors[$key] ) && !$cralv->isPostCode( $data[$key] ) ) {
    ...
    {
        unset($error);
        $userInfo = $this->app->authManager->getUserInfo();

        $altUserInfo = $this->app->authManager->getAlternativeUserInfo();

        // タイトル文字取得
        $title = $this->altUserInfo ?
        sprintf( '%s %s(%s)　様', $userInfo->NameSeiKj, $userInfo->NameMeiKj, $altUserInfo->NameKj ) :
        sprintf( '%s%s　様', $userInfo->NameSeiKj , $userInfo->NameMeiKj );
        ...
        $PassHistCnt = $mdlph->findnew(5, $userInfo->LoginId);
        $PassHist = $mdlph->findnew(5, $userInfo->LoginId)->current();
        ...
        $this->view->assign('userInfo', $title);
        $this->view->assign('mode', $loginmode);
        $this->view->assign('msg', $msg);
        $this->view->assign('mailAddress', $userInfo->MailAddress);
        ...
        if ($cmd == 'e') {
            ...
            //過去4回分のパスワードを取得
            $sql = " SELECT LoginPasswd FROM T_MypagePasswordHistory WHERE Category = 5 AND LoginId = :LoginId ORDER BY Seq DESC LIMIT " . $passTimeLimit;
            ...
            if( !(empty($oldPass)) && $oldPassHashed != $userInfo->LoginPasswd )
            {
                $error[] = '現在のパスワードに誤りがあります。';
            }
            ...
            if ( !(empty($newPass1)) && (!preg_match('/[a-z]/', $newPass1) || !preg_match('/[A-Z]/', $newPass1) || !preg_match('/[0-9]/', $newPass1)
            ...
            else
            {
                ...
                $mypageCustomer = new TableMypageCustomer($this->app->dbAdapter);
                $mypageCustomer->saveUpdate( $saveData, $userInfo->CustomerId );
