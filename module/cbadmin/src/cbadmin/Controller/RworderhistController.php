...
class RworderhistController extends CoralControllerAction
{
	...
	public function _init()
	{
        ...
        $this->addStyleSheet('../css/cbadmin/rworder/detail/default.css');
        ...
        $this->setPageTitle("後払い.com - 履歴照会");
	}
	...
	public function detailAction()
	{
        ...
        if (empty($others['MailInfo'])){ $others['MailInfo'] = array(); }
        ...
        // (請求件数)
        $others['claimCount'] = (is_null($row['CLM_F_ClaimDate']) ? 0 : 1) + $aryLen;
        // (ステータス)
        ...
        //ステータスにExtraPayTypeを追加
        $others['Status']['ExtraPayType'] = isset($others['Reserve']['ExtraPayType']) ? $others['Reserve']['ExtraPayType'] : null;
        ...
        // 入金方法
        if (! empty ( $row ['REC_ReceiptClass'] )) {
            // コードマスターから入金方法のコメントを取得
            $mdlc = new TableCode ( $this->app->dbAdapter );
            $row['KeyContent'] = $mdlc->find ( 198, $row ['REC_ReceiptClass'] )->current ()['KeyContent'];
        }
