...
/**
 * {@link EnterpriseCsvWriter}の出力・変換設定を定義するクラス。
 */
class EnterpriseCsvSettings {
	/**
	 * コンバータクラスの名前空間
	 * @var string
	 */
	const CONVERTER_NAMESPACE = 'Coral_Converter';

	/**
	 * 指定パスにあるJSON形式の設定ファイルから、
	 * {@link EnterpriseCsvSettings}の新しいインスタンスを生成する
	 * @static
	 * @param string 設定ファイルのパス
	 * @return EnterpriseCsvSettings このクラスの新しいインスタンス
	 */
	...
	/**
	 * {@link EnterpriseCsvSettings}の新しいインスタンスを初期化する
	 */
	 ...
	 *  - field : 入力元のT_Enterpriseのカラム名。内部連想配列のキーとしても採用される
	 *  - title : 出力時のカラムヘッダに使用するラベル文字列
	 *  - converterName : コンバータクラスの名前。実際のコンバータクラスの名前空間'Coral_Converter'を除いた形式
	 *  - args : コンバータクラスのコンストラクタパラメータの配列
	 *  - disabled : 無効設定フラグ。trueを指定した場合、その設定は使用されない
	 * @param array $setting 設定情報を格納した連想配列
	 * @return EnterpriseCsvSettings このインスタンス自身
	 */
	public function addSetting($setting) {
	    if( is_array($setting) ) {
			...
			$converterClass = BaseIOUtility::buildPath(
				...
				empty($setting['converterName']) ? 'Default' : $setting['converterName'],
				'_' );
			...
			if( ! $setting['disabled'] ) {
			    ...
			    $ref = new \Coral\Coral\Converter\CoralConverterDefault();
				$converter = $ref->create();
	 ...
	 * キーは、入力データとなるT_Enterpriseのカラム名に一致する
	 * @param string $key コンバータキー
	 * @return Coral_Converter_Interface $keyに関連付けられたコンバータインスタンス
	 */
