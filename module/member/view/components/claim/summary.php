...
<div class="layout_container">
	<div class="print_header">
		...
		<div id="issue_message">
			...
			立替締め日：<?php echo f_df($this->fixedDate, 'Y/m/d') . ' (' . f_df($this->summary['ExecScheduleDate'], 'y/m/d') . ' 支払分)'; ?>　のご利用額は以下の通りです。
		</div>
	...
	<table class="claim_result charge_list_table summary_table" border="1" cellpadding="0" cellspacing="0">
		<tbody>
            ...
            <tr class="summary fat_border">
				<td class="seq_col label_col summary_label_col"><div>立替締め日</div></td>
				<td class="date_col total_col summary_value_col"><div><?php echo $p['FixedExecDate']; ?></div></td>
			</tr>
        <?php if ($p['ChargeCountExtraPay1DK'] > 0 || $p['ChargeCountExtraPay1BS'] > 0 ) { ?>
            <tr class="summary">
                ...
                <td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf($p['ChargeCount'], '#,##0 件'), '0 件'); ?></div>
                <?php if ($p['ChargeCountExtraPay1DK'] > 0) { ?>
        ...
        <?php } else { ?>
            <tr class="summary">
                ...
                <td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf($p['ChargeCount'], '#,##0 件'), '0 件'); ?></div></td>
            </tr>
        <?php } ?>
			<tr class="summary fat_border">
				<td class="seq_col label_col summary_label_col"><div>ご利用総額</div></td>
				<td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf($p['ChargeAmount'], '\ #,##0'), '\ 0'); ?></div></td>
			</tr>
        <?php if ($p['ChargeCountExtraPay1DK'] > 0 || $p['ChargeCountExtraPay1BS'] > 0 ) { ?>
            <tr class="summary">
		...
		<?php } ?>
			...
			<tr class="summary">
				...
				<td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf(-1 * (int)$p['StampFeeTotal'], '\ #,##0'), '\ 0'); ?></div></td>
			</tr>
			<tr class="summary">
			...
			<tr class="summary">
				...
				<td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf($p['CarryOver'], '\ #,##0'), '\ 0'); ?></div></td>
			</tr>
			<tr class="summary">
				<td class="seq_col label_col summary_label_col"><div>キャンセル返金分</div></td>
                <?php if ($p['CalcelAmountExtraPay1DK'] > 0 && $p['CalcelAmount'] > 0) { ?>
                  <div>（内 届いてから払い合計（同梱））<div>
                <?php } ?>
                <?php if ($p['CalcelAmountExtraPay1BS'] > 0 && $p['CalcelAmount'] > 0) { ?>
                  <div>（内 届いてから払い合計（別送））</div>
                <?php } ?>
				<td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf($p['CalcelAmount'], '\ #,##0'), '\ 0'); ?></div></td>
                <?php if ($p['CalcelAmountExtraPay1DK'] > 0 && $p['CalcelAmount'] > 0) { ?>
			...
			<tr class="summary fat_border">
				...
				<td class="money_col total_col summary_value_col"><div><?php echo nvl(f_nf(-1 * (int)$p['TransferCommission'], '\ #,##0'), '\ 0'); ?></div></td>
			</tr>
<?php if (false) { ?>
