...
";
	 ...
	 * @return array 検証結果
	 */
	public function validateForEdit(array $input, $mail_required = false) {
		...
		if( $modifiability[self::GROUP_CUSTOMER] ) {
			$errors['Customer'] = $this->validateCustomerGroup($input['Customer'], $mail_required, $input['Order']['CreditTransferRequestFlg']);
		}
		...
		if( $modifiability[self::GROUP_ITEMS] ) {
			$errors['Items'] = $this->validateItemsGroup($input['Items']);
		}
     ...
     * @return array
     */
    public function validateCustomerGroup(array $input, $mail_required = false , $CreditTransferRequestFlg) {
	 ...
	 * @return array
	 */
	public function validateItemsGroup(array $input) {
