...
class ShippingController extends CoralControllerAction {
    ...
    protected function _init() {
        ...
        $this
            ...
            ->addStyleSheet( './css/shipping.css' )
            ->addStyleSheet( './css/shipping_sp.css' )
...
Q_END;
    ...
    public function registAction() {
        ...
        $this->addStyleSheet( '../css/base.ui.modaldialog.css' )
             ->addStyleSheet( './css/shipping_regist.css' )
             ->addStyleSheet( '../css/base.ui.tableex.css' )
             ->addStyleSheet( '../css/base.ui.customlist.css' )
             ->addStyleSheet( './css/search_result.css' )
        ...
        $cntOrderList = 0;
        if( !empty($order_list) ){
            $cntOrderList = count( $order_list );
        }
        $this->view->assign( 'list_count', $cntOrderList );
    ...
    public function confirmAction() {
        $this->setPageTitle( '入力内容の確認' )
            ->addStyleSheet( './css/shipping_confirm.css' );
...
//                     }
// Del By Takemasa(NDC) 20151203 Del メール便(伝票番号登録区分が[0：不要]な配送方法時)の着荷確認を取らない
                    if( $back == 'changeCsv' ) {
...
// Del By Takemasa(NDC) 20160222 End 伝票番号修正時はT_PayingAndSales更新しない
                        ...
                        $seq = $payingTable->saveNew( $payRow );        // 2015/10/06 Y.Suzuki 会計対応 Mod
...
//                }
    ...
    public function downloadAction() {
        ...
        $templateId = 'CKA03011_2';    // 個別配送伝票一覧CSV
        $templateClass = 2;
    ...
    public function confirmCsvAction() {
        ...
        ->addStyleSheet( './css/shipping_confirm_csv.css' );
        ...
        if( ! preg_match( '/\.xl.$/i', $_FILES[ self::UPLOAD_FIELD_NAME ]['name'] ) && $csv != "" ) {
            ...
            if( $success != false ) {
                ...
                if(!empty($handler->getResults())){
                    $validCount = count($handler->getResults());
                }
                if(!empty($handler->getExceptions())){
                    $errorCount = count($handler->getExceptions());
                }
                if(!empty($handler->getWarnings())){
                    $warningCount = count($handler->getWarnings());
                }
                if(!empty($handler->getAllResults())){
                    $totalCount = count($handler->getAllResults());
                }
                ...
                $this->view->assign( 'headerRow', $headerRow );
                $this->view->assign( 'validRows', $handler->getResults() );
                $this->view->assign( 'validCount', $validCount );
                $this->view->assign( 'errorRows', $handler->getExceptions() );
                $this->view->assign( 'warningRows', $handler->getWarnings() );
                $this->view->assign( 'errorCount', $errorCount + $warningCount );
                $this->view->assign( 'allData', $handler->getAllResults() );
                $this->view->assign( 'totalCount', $totalCount);
                $this->view->assign( 'caution', $caution );
                $this->view->assign( 'csvCaution', $csvCaution);
