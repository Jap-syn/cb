...
class OrderController extends CoralControllerAction {
	...
	protected function _init()
	{
        ...
        $this
            ...
            ->addStyleSheet( './css/order_form.css' )
...
EOQ;
        ...
        $entdata = $this->app->dbAdapter->query($sql)->execute(array(':EnterpriseId' => $enterpriseId))->current();
        $creditTransferFlg = $entdata['CreditTransferFlg'];
        $billingAgentFlg   = $entdata['BillingAgentFlg'];

        // フォームマネージャの構築
        $validationMap = array();
        $this->formManager = OrderInputInfo::createFormManager($validationMap, $creditTransferFlg, $billingAgentFlg);
        ...
        $this->view->assign( 'BillingAgentFlg', $billingAgentFlg);
...
//        }
        ...
        foreach( $this->formManager->getAllItems() as $item ) {
            ...
            // 自由入力項目１～２０ 入力桁数チェック
            if( mb_substr($key, 0, 4) == 'Free' ) {
                $chkTxt = mb_convert_encoding($posts[$key], 'sjis-win', 'UTF-8');
                if (strlen($chkTxt) > 50) {
                    $this->app->logger->debug('$key='. $key. '、'. str_replace('Free', '', $key));
                    $orderAddErrors[] = (double)str_replace('Free', '', $key);
                }
            }
        }
        ...
        if( !empty($errors) || !empty($itemErrors) || !empty($orderAddErrors) ) {
            $messages = array( '以下の項目のデータに誤りがあります。' );
            foreach( $errors as $error_item ) {
                $messages[] = '・' . $error_item->getCaption();
            }
            foreach( $orderAddErrors as $free_num ) {
                $messages[] = "・自由入力情報 ( $free_num 行目 )";
            }
            ...
            $this->view->assign( 'orderAddErrors', $orderAddErrors );
        ...
        $this->addStyleSheet( './css/order_confirm.css' );
	...
	public function saveAction()
	{
        ...
        try {
            ...
            $seq = $this->saveOrderData($arrays, $userId, 0);
...
EOQ;
            ...
            $entdata = $this->app->dbAdapter->query($sql)->execute(array(':EnterpriseId' => $this->app->authManager->getUserInfo()->EnterpriseId))->current();
            $billingAgentFlg   = $entdata['BillingAgentFlg'];

            // 注文_追加情報へ登録
            if ($billingAgentFlg == 1) {
                $odraddinfo = new TableOrderAddInfo($this->app->dbAdapter);
                $OdrAddInfArray = array(
                        'OrderSeq'   => $seq,
                        'Free1'      => $posts['Free1'],
                        'Free2'      => $posts['Free2'],
                        'Free3'      => $posts['Free3'],
                ...
                $odraddinfo->saveNew($OdrAddInfArray);
            }
	...
	public function confirmCsvAction()
	{
        ...
        if(! preg_match( '/\.xl.$/i', $fileName ) &&  $file['tmp_name'] != "") {
            ...
            if(!empty($handler->getAllResults())) {
                $totalCount = count( $handler->getAllResults() );
            }

            $totalOrderCount = 0;
            if(!empty($handler->getBuilders())) {
                $totalOrderCount = count( $handler->getBuilders() );
            }
...
//            $handler->hyphenNormalize($handler->getCsvSchema());
            ...
            $this->view->assign( 'totalOrderCount', $totalOrderCount );
            $this->view->assign( 'caution', $caution);
...
//	}
        ...
        if( isset( $session->dbRows ) ) {
            ...
            try {
                ...
                foreach( $session->dbRows as $db_row ) {
                    ...
                    $newseq = $this->saveOrderData($db_row, $userId, 1);
    ...
    public function editAction()
    {
        ...
        if($input) {
            ...
            //注文追加情報
            if($modifiability[OrderEditor::GROUP_ADDINFO]){
                foreach($input['OrderAddInfo'] as $key => $value){
                    $row[$key] = $value;
                }
            }
            $itemList = $items;
...
EOQ;
        ...
        $row = $stm->execute($prm)->current();
	        ...
	        //注文追加情報
        $sql = "SELECT *
                FROM T_OrderAddInfo OAI
                WHERE OrderSeq = :OrderSeq";

        $stm = $this->app->dbAdapter->query($sql);

        $rowAddInfo = $stm->execute(array(':OrderSeq' => $row['OrderSeq']))->current();

        if(!empty($rowAddInfo)){
            //自由入力情報以外を除外
            $rowFree = array();
            foreach($rowAddInfo as $key => $value){
                if(strpos($key, 'Free') === 0){
                    $rowFree[$key] = $rowAddInfo[$key];
                }
            }
            $row = array_merge($rowFree, $row );
        }
...
EOQ;
    ...
    public function editconfirmAction()
    {
        ...
        $validate_results = $editor->validateForEdit($input, $mail_required);
...
EOQ;
            ...
            if ($modifiability[OrderEditor::GROUP_ADDINFO]) {
                ...
                $sql = "SELECT COUNT(1) AS cnt FROM T_OrderAddInfo WHERE OrderSeq = :OrderSeq";

                $cnt = $this->app->dbAdapter->query($sql)->execute(array(':OrderSeq'=>$order_seq))->current()['cnt'];
                $odraddinfo = new TableOrderAddInfo($this->app->dbAdapter);
                $OdrAddInfArray = array(
                        'Free1'      => $input['OrderAddInfo']['Free1'],
                        'Free2'      => $input['OrderAddInfo']['Free2'],
                        'Free3'      => $input['OrderAddInfo']['Free3'],
                        'Free4'      => $input['OrderAddInfo']['Free4'],
                        'Free5'      => $input['OrderAddInfo']['Free5'],
                        'Free6'      => $input['OrderAddInfo']['Free6'],
                        'Free7'      => $input['OrderAddInfo']['Free7'],
                        'Free8'      => $input['OrderAddInfo']['Free8'],
                        'Free9'      => $input['OrderAddInfo']['Free9'],
                        'Free10'     => $input['OrderAddInfo']['Free10'],
                        'Free11'     => $input['OrderAddInfo']['Free11'],
                        'Free12'     => $input['OrderAddInfo']['Free12'],
                        'Free13'     => $input['OrderAddInfo']['Free13'],
                        'Free14'     => $input['OrderAddInfo']['Free14'],
                        'Free15'     => $input['OrderAddInfo']['Free15'],
                        'Free16'     => $input['OrderAddInfo']['Free16'],
                        'Free17'     => $input['OrderAddInfo']['Free17'],
                        'Free18'     => $input['OrderAddInfo']['Free18'],
                        'Free19'     => $input['OrderAddInfo']['Free19'],
                        'Free20'     => $input['OrderAddInfo']['Free20'],
                );
                //注文追加情報が存在する場合レコードを更新
                if(0 < $cnt){
                    $odraddinfo->saveUpdate($OdrAddInfArray, $order_seq);
                //注文追加情報が存在しない場合レコードを追加
                }else{
                    $OdrAddInfArray['OrderSeq'] = $order_seq;
                    $odraddinfo->saveNew($OdrAddInfArray);
                }
            }
            $this->app->dbAdapter->getDriver()->getConnection()->commit();
	 ...
	 * @param boolean $csvflg CSVフラグ
	 * @return int 注文シーケンス番号
	 */
	private function saveOrderData($data, $userId, $csvflg)
	{
        ...
        if ( $ent['BillingAgentFlg'] == '1' && $csvflg == 1 ) {
            $mdloai = new TableOrderAddInfo($this->app->dbAdapter );
            $oaiPrm = array();
            $oaiPrm['OrderSeq'] = $orderSeq;
            for ($idx = 1; $idx< 21; $idx++ ) {
                $oaiPrm['Free'. $idx] = $data[ OrderInputInfo::ARRKEY_ORDER ]['Free'. $idx];
            }
            $mdloai->saveNew($oaiPrm);
        }
	...
	private function createOrderArraysFromPost($postData)
	{
		...
		echo $this->postData['I_UnitPrice_' . $this->order_item_carriage__];
		return $arrays;
	}
...
//            $db_rows = $this->hyphenNormalize($db_rows);
    ...
    private function modifyCsvOrderData($data, $userId, &$retIsSaiyoshin)
    {
        ...
        if ( $ent['BillingAgentFlg'] == '1' ) {
            $mdloai = new TableOrderAddInfo($this->app->dbAdapter );
            $oaiPrm = array();
            $oaiPrm['OrderSeq'] = $order['OrderSeq'];
            for ($idx = 1; $idx< 21; $idx++ ) {
                $oaiPrm['Free'. $idx] = $data[ OrderInputInfo::ARRKEY_ORDER ]['Free'. $idx];
            }
            if ( $mdloai->find( $order['OrderSeq'] )->count() > 0 ) {
                $mdloai->saveUpdate($oaiPrm, $order['OrderSeq']);
            } else {
                $mdloai->saveNew($oaiPrm);
            }
        }
    ...
    protected function IsSaiyoshin($data, $order) {
        ...
        // 再与信となるかどうか、変更をチェック(購入者に対して) ※検証項目[NameKj／PostalCode／UnitingAddress／Phone／MailAddress]
...
EOQ;
        ...
        $entdata = $this->app->dbAdapter->query($entsql)->execute(array(':EnterpriseId' => $this->app->authManager->getUserInfo()->EnterpriseId))->current();
        $billingAgentFlg   = $entdata['BillingAgentFlg'];

        // 注文_追加情報へ登録
        if ($billingAgentFlg == 1) {
            $mdloai = new TableOrderAddInfo($this->app->dbAdapter);
            $oaiData = $mdloai->find($row['OrderSeq'])->current();
            $i = 1;
            if(!empty($oaiData)){
                for ( $i ; $i <= 20 ; $i++ ){
                    $row['Free' . $i] = $oaiData['Free' . $i];
                }
            }
        }
    ...
    public function defectlistAction()
    {
        ...
        $this->addStyleSheet( './css/defectlist.css' );
