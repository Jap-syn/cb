...
use member\classes\SearchExpressionInfo;
...
EOQ;
	...
	public function searchAction() {
        $this
            ...
            ->addJavaScript( './js/searchcolumneditor.js' );
...
//$this->app->logger->info($q);
	...
	public function detailAction() {
        ...
        $this
            ->setPageTitle( '取引情報詳細' )
            ->addStyleSheet( './css/search_detail.css' )
            ->addStyleSheet( '../css/base.ui.modaldialog.css' )
            ->addStyleSheet( './css/search_result.css' )
            ->addJavaScript( '../js/base.ui.customlist.js')
            ->addJavaScript( '../js/base.ui.modaldialog.js' );
...
EOQ;
		...
		// 自由入力項目の取得
		if ($rows[0]['BillingAgentFlg'] == 1){
		    $mdloai = new TableOrderAddInfo( $this->app->dbAdapter);
		    $OdrAddInfdata = $mdloai->find($rows[0]['OrderSeq'])->current();
		    $this->view->assign( 'OdrAddInfo', $OdrAddInfdata);
		}
...
// Del By Takemasa(NDC) 20150126 End マジックメソッド廃止
	...
	private function buildSearchExpressions(array $params) {
		...
		foreach( $params as $id => $value ) {
		    ...
		    $receiptCount = 0;
		    if(!empty($this->_lists['IsWaitForReceipt'])){
		        $receiptCount = count($this->_lists['IsWaitForReceipt']);
		    }
		    // 伝票番号未登録の場合はここで判定
		    ...
		    else if( $id == 'IsWaitForReceipt' && $valueCount >= $receiptCount) {
		        // 入金状態全指定時は条件追加しない
		        continue;
		    }
