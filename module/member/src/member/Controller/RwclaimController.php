...
class RwclaimController extends CoralControllerAction
{
	...
	const SESSION_JOB_PARAMS = 'MRWCLAIM_JOB_PARAMS';
	...
	protected function _init()
	{
        ...
        $this->addStyleSheet( './css/rwclaim_member.css');
...
EOQ;
    ...
    public function changecsvAction() {
        ...
        //項目の設定
        if($tId == 'CKA04016_1' && date('Y-m-d') > '2019-09-30'){
    ...
    /**
     * ジョブ転送処理を行う
     * @param string $msg 処理結果メッセージ
     * @return boolean
     */
    protected function jobTransfer() {
        ...
        $executed_oseqs = array();// Add By Takemasa(NDC) 20160122 重複注文SEQﾌﾞﾛｯｸ(緊急対応)
        while (isset($params['P_OrderSeq' . $i])) {
            if (!isset($params['chkPrintCsv' . $i])) { $i++; continue; }
            $oseq = $params['P_OrderSeq' . $i];

            // Add By Takemasa(NDC) 20160122 Stt 重複注文SEQﾌﾞﾛｯｸ(緊急対応)
            if (in_array($oseq, $executed_oseqs)) {
                $this->app->logger->err('Critical-Warning : ' . $oseq);
                $i++;
                continue;
            }
            else {
            ...
            // Add By Takemasa(NDC) 20160122 End 重複注文SEQﾌﾞﾛｯｸ(緊急対応)
...
EOQ;
     ...
     * @return boolean
     */
    protected function jobTransfer2() {
...
EOQ;
    ...
    protected function pdfDownload() {
        ...
        while( isset( $params['P_OrderSeq' . $i] ) ) {
            ...
            // その他点数
            $data['ItemsCountEtc'] = $itemCount - 19;
