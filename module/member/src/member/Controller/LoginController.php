...
class LoginController extends CoralControllerAction {
	...
	public function authAction() {
		...
		if(preg_match('/^([^:]+)(::?)(.+)$/', $id, $userIdInfo)) {
			...
			if ($PassHistCnt > 0 && $PassHist['PasswdLimitDay'] < date('Y-m-d')) {
				$this->messenger->addMessage( '代理ログインユーザーのパスワード有効期限切れです。</br>CB管理画面でパスワードを変更してください。' );
    ...
    // 不正アクセスIP判定処理
		...
		if ($agencyflg == 0) {
		    ...
		    // 最新の有効なパスワード履歴データ
		    $PassHist = ResultInterfaceToArray($mdlph->findnew(2, $id));
		    // パスワード期限警告日数
		...
		if($agencyflg == 0 && !empty($PassHist) && $PassHist[0]['PasswdLimitDay'] >= date('Y-m-d') && $PassHist[0]['PasswdLimitDay'] <= date('Y-m-d', strtotime($passAlert ." day"))){
	...
	public function chgpwAction()
	{
	    ...
	    // $PassHistCnt = $mdlph->findnew(2, $userInfo->LoginId);
	    $PassHist = $mdlph->findnew(2, $userInfo->LoginId)->current();
	    ...
	    if ($cmd == 'e') {
	        ...
	        $newPass2 = $ar['npw2'];
 	        ...
 	        //過去4回分のパスワードを取得
 	        $sql  = " SELECT LoginPasswd FROM T_PasswordHistory WHERE Category = 2 AND LoginId = :LoginId ORDER BY Seq DESC LIMIT " . $passTimeLimit;
 	        ...
 	        if (!preg_match('/[a-z]/', trim($newPass1)) || !preg_match('/[A-Z]/', trim($newPass1)) || !preg_match('/[0-9]/', trim($newPass1)) || !preg_match('/[ -\/:-@\[-`\{-\~]/', trim($newPass1)) || $pass_len < $passMin)
 	        {
