...
class LogicChargeDecision
{
     ...
     * @param array $transCsvList 振込データCSVファイルパスの配列(未使用)
     * @param array $claimPdfList 都度請求PDFファイルパスのリスト(未使用)
    ...
    public function decision($oemId, $transCsvList, $claimPdfList, $userId)
    {
        ...
        foreach ($payingSeqList as $payingSeq) {
            ...
            // 精算停止区分がONの場合は、立替確定処理をスキップ
            $execStopFlg = $mdle->find($enterpriseId)->current()['ExecStopFlg'];
            if ($execStopFlg == 1) {
                continue;
            }
...
EOQ;
            ...
            // 更新対象の立替振込管理Seqで加盟店月締め情報テーブルを検索
            $sql = " SELECT * FROM AT_EnterpriseMonthlyClosingInfo WHERE PayingControlSeq = :PayingControlSeq ";
            $ri = $this->_adapter->query($sql)->execute(array(':PayingControlSeq' => $payingSeq));
            // 1件でも存在していた場合、加盟店月締め情報テーブルへの更新処理を行う。
            if ($ri->count() > 0) {
                // 加盟店月締め情報テーブルの更新
                $sql = "";
                $sql .= " UPDATE AT_EnterpriseMonthlyClosingInfo ";
                $sql .= " SET    PayingControlAddUpFlg = 1 ";
...
// ↓↓↓MySQL Bug# 25053286 対応のため、コメントアウト
//             // 注文履歴登録用に注文Seqを取得する。
//             // 立替振込管理Seqで検索
//             $conditionArray = array('PayingControlSeq' => $payingSeq);
...
// ↑↑↑MySQL Bug# 25053286 対応のため、コメントアウト
	...
	public function payTempFixed(){
        ...
        try {
            ...
            foreach ($eDatas as $eData){
                ...
                $chargeMonthlyFeeFlg = 0;  // 月額固定費を計上したか否か
                ...
                if ($cnt <= 0) {    // 今月すでに計上されていない場合のみ
                    ...
                    $sql = "SELECT * FROM AT_EnterpriseMonthlyClosingInfo WHERE EnterpriseId = :EnterpriseId AND ClosingMonthly = :ClosingMonthly AND PayingControlAddUpFlg = 0 ";
                ...
                $payingControlSeq = $mdlpc->saveNew(
                    array(
                        ...
                        'ChargeMonthlyFeeFlg' => $chargeMonthlyFeeFlg,                  // 月額固定費課金
                ...
                // 加盟店月締め情報の更新(月額固定費を計上したときのみ)
                if ($chargeMonthlyFeeFlg == 1) {
                    $this->setPayingControlSeq($payingControlSeq, $enterpriseId, $closingMonthly, $opId);
                }
	...
	{
        $seqs = $this->_adapter->query(" SELECT IFNULL(GROUP_CONCAT(Seq),'0') AS Seqs FROM T_PayingAndSales WHERE PayingControlSeq = :PayingControlSeq AND PayingControlStatus = 0 ")->execute(
            array(':PayingControlSeq' => $payingControlSeq))->current()['Seqs'];

        $sql = <<<EOQ
UPDATE T_PayingAndSales
SET    ChargeDecisionFlg  = 0
,      ChargeDecisionDate = NULL
,      PayingControlSeq   = NULL
,      UpdateDate         = :UpdateDate
,      UpdateId           = :UpdateId
WHERE  Seq IN ($seqs)
EOQ;
        return $this->_adapter->query($sql)->execute(array(':UpdateDate' => date('Y-m-d H:i:s'), ':UpdateId' => $opId));
	}
...
SET    Chg_Status       = 0
,      Chg_FixedDate    = NULL
,      Chg_DecisionDate = NULL
,      Chg_ChargeAmount = NULL
,      Chg_Seq          = NULL
,      UpdateDate       = :UpdateDate
,      UpdateId         = :UpdateId
WHERE  Chg_Seq          = :Chg_Seq
EOQ;
        return $this->_adapter->query($sql)->execute(array(':UpdateDate' => date('Y-m-d H:i:s'), ':UpdateId' => $opId, ':Chg_Seq' => $payingControlSeq));
	}
...
EOQ;
	...
	{
        $sql = <<<EOQ
SELECT IFNULL(GROUP_CONCAT(can.Seq),'0') AS Seqs
FROM   T_Order o
       INNER JOIN T_Cancel can ON (can.OrderSeq = o.OrderSeq)
WHERE  can.ApproveFlg = 1
AND    can.KeepAnAccurateFlg = 0
AND    can.ValidFlg = 1
AND    DATE(can.ApprovalDate) <= :ApprovalDate
AND    o.EnterpriseId = :EnterpriseId
EOQ;
        $seqs = $this->_adapter->query($sql)->execute(array(':ApprovalDate' => $fixedDate, ':EnterpriseId' => $enterpriseId))->current()['Seqs'];

        $sql = <<<EOQ
UPDATE T_Cancel
SET    KeepAnAccurateFlg  = 1
      ,KeepAnAccurateDate = :KeepAnAccurateDate
      ,PayingControlSeq   = :PayingControlSeq
      ,UpdateDate         = :UpdateDate
      ,UpdateId           = :UpdateId
WHERE Seq IN ($seqs)
...
EOQ;
	...
	private function setAdjustmentAmount($payingControlSeq, $enterpriseId, $opId)
	{
	    ...
	    $arrAdjustAmountCount = 0;
	    if (!empty($this->_arrAdjustAmount)) {
	        $arrAdjustAmountCount = count($this->_arrAdjustAmount);
	    }

        for ($i = 0; $i < $arrAdjustAmountCount; $i++){
     ...
     * @param int $payingControlSeq 立替振込管理Seq
     * @param int $enterpriseId 加盟店ID
     * @param date $closingMonthly 計上月
     * @param int $opId 担当者
     */
    private function setPayingControlSeq($payingControlSeq, $enterpriseId, $closingMonthly, $opId) {
        // UPDATE文構築
        $sql = "";
        $sql .= " UPDATE AT_EnterpriseMonthlyClosingInfo ";
        $sql .= " SET    PayingControlSeq = :PayingControlSeq ";
        $sql .= "    ,   UpdateDate = :UpdateDate ";
        $sql .= "    ,   UpdateId = :UpdateId ";
        $sql .= " WHERE  EnterpriseId = :EnterpriseId ";
        $sql .= " AND    ClosingMonthly = :ClosingMonthly ";
        $sql .= " AND    PayingControlAddUpFlg = 0 ";
