...
/**
 * JNB関連のシステム設定を管理するためのロジッククラス
 */
class LogicJnbConfig extends LogicJnbCommon {
    ...
    /**
     * DBアダプタを指定してLogicJnbConfigの新しいインスタンスを初期化する
     *
    ...
    public function getTargetCategory() {
        return TableSystemProperty::FIX_CATEGORY_JNBCONF;
    }
        ...
        foreach($this->_getDefaultConfig() as $prop => $value) {
            if(!$sysProp->propNameExists($prop, $this->getTargetCategory())) {
                $data = array(
                    ...
                    'Name' => $prop,
                    'PropValue' => $value,
                    'Description' => sprintf('JNB設定：%s', $this->_getPropDescription($prop))
                );
    ...
    /**
     * JNB設定で使用するプロパティ名とそのデフォルト値一式を取得する
     *
     * @access protected
     * @return array
     */
    protected function _getDefaultConfig() {
        return array(
            // デフォルトの銀行名
            self::PROP_DEFAULT_BANK_NAME => 'ジャパンネット銀行',

            // デフォルトの金融機関コード
            self::PROP_DEFAULT_BANK_CODE => '0033',
    ...
    protected function _getValue($propName) {
        ...
        $value = trim(nvl($this->getSystemPropertyTable()->getValue(
            ...
            \models\Table\TableSystemProperty::FIX_CATEGORY_JNBCONF,
            $propName
        )));
