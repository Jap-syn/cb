...
class LogicJnbAccountImporter extends LogicJnbCommon {
     ...
     * LogicJnbAccountImporterの新しいインスタンスを初期化する
     *
     ...
     *
     * @param int $oemId OEM ID
     * @return LogicJnbAccountImporter このインスタンス
     */
    ...
    /**
     * 現在処理対象となっているJNB契約情報のサマリーを取得する
     *
     * @return array
     */
	public function getCurrentJnbSummary() {
        $oid = $this->getTargetOemId();
        return $this->getJnbTable()->findSummaryByOemId($oid);
	}
...
$this->debug('[loadCsv] called.');
		...
		// 現在のJNB契約のサマリを取得
		$data = $this->getCurrentJnbSummary();
		if(!$data) {
            throw new \Exception('JNB契約情報が取得できません。OEM指定が不正です');
		}

		// トランザクションパラメータを今回の処理向けに初期化
		$this->_csvContext = array(
            'key' => null,
			'jnbId' => (int)$data['JnbId'],
...
$this->info(sprintf('[loadCsv] end load ABNORMALY !! elapsed time = %s, error = ', (microtime(true) - $start), $err->getMessage()));
     ...
     *               'groupId'      -> インポート先のJNB口座グループID。同一プロセスキーに対する処理で最初の要求以外は
...
$this->debug(sprintf('[execImport:%s] called. limit = %s, group = %s', $processKey, $limit, $groupId));
		...
		$grpTable = $this->getJnbGroupTable();
...
$this->debug(sprintf('[cancelImport:%s] method called. target processKey = %s', $processKey, $processKey));
        $this->_adapter->getDriver()->getConnection()->beginTransaction();
        try {
            $sql = " DELETE FROM T_JnbAccountImportWork WHERE ProcessKey = :ProcessKey ";
...
EOQ;
     ...
     * @return ResultInterface JNB口座インポート作業テーブルの行イメージ
     */
...
$this->debug(sprintf('[clearCsvErrors:%s] method called. target processKey = %s', $processKey, $processKey));
        $sql = " DELETE FROM T_JnbAccountImportWork WHERE ProcessKey = :ProcessKey AND CsvError IS NOT NULL ";
        $ri = $this->_adapter->query($sql)->execute(array(':ProcessKey' => $processKey));
        $count = $ri->getAffectedRows();
...
$this->debug(sprintf('[clearImportErrors:%s] method called. target processKey = %s', $processKey, $processKey));
        $sql = " DELETE FROM T_JnbAccountImportWork WHERE ProcessKey = :ProcessKey AND ImportError IS NOT NULL ";
        $ri = $this->_adapter->query($sql)->execute(array(':ProcessKey' => $processKey));
        $count = $ri->getAffectedRows();
...
$this->info(sprintf('[csvReadCallback:%08d] col = %s, val = %s, rule = %s, is_date = %s', $lineNum, $colName, $value, $rule, $colInfo[$colName]['is_date'] ? 'YES' : 'NO'));
     ...
     * CSVのスキーマ通りの並び順でJNB口座／JNB口座インポート作業のテーブルで定義されている対応する
     * カラム名をキーとして、値には表示名、検証ルールの正規表現およびJNB口座へのインポート対象で
     * あるかを示すフラグを備える
     *
     * @return array
     */
	public function getImportCsvSchema() {
		...
		return array(
			...
			'JnbStatus'			=> array('label' => 'ステータス',		'match' => '/^.{1,255}$/u', 'to_imp' => 1),
