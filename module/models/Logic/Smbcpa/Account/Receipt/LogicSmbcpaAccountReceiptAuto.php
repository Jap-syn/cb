...
/**
 * SMBCバーチャル口座の自動入金入金処理ロジック
 */
class LogicSmbcpaAccountReceiptAuto extends LogicSmbcpaAccountReceipt {
    /**
...
$this->err($err->getTraceAsString());
    ...
    protected function _receipt(array $nfData, $userId) {
        ...
        // 上位SMBCバーチャル口座契約の有効チェック
        if(!$this->getSmbcpaTable()->find($account['SmbcpaId'])->current()['ValidFlg']) {
            // 無効なSmbcpa IDの場合
            return $this->doneUnreceipt($seq, self::RESULT_TYPE_10_DISABLED_SMBCPA_ID, $oseq, $accSeq);
        }
        // 上位グループの返却状態をチェック
        if($this->getSmbcpaGroupTable()->find($account['AccountGroupId'])->current()['ReturnedFlg']) {
            // 返却済み口座の場合
