...
/**
 * SMBCバーチャル口座の入金処理抽象ロジック
 */
abstract class LogicSmbcpaAccountReceipt extends LogicSmbcpaCommon {
    ...
    /** 入金不能事由定数：無効なSmbcpa ID @var int */
    const RESULT_TYPE_10_DISABLED_SMBCPA_ID = 10;

    /** バッチ入金処理で使用するスレッドプールのスレッドグループ名 @var string */
    const BATCH_THREAD_GROUP_NAME = 'smbcpa-auto-rcpt-batch';
    ...
    protected static function getResultTypeSettings() {
        return array(
            ...
            // 10：無効なSmbcpa ID
            self::RESULT_TYPE_10_DISABLED_SMBCPA_ID =>
                array('msg' => '無効なSMBCバーチャル口座 ID', 'result' => self::RCPT_RESULT_EXEMPT, 'status' => -1),
    ...
    /**
     * SMBCバーチャル口座ロジックのインスタンスを取得する
     *
     * @return LogicSmbcpaAccount
     */
    public function getAccountLogic() {
        return new LogicSmbcpaAccount($this->getDbAdapter());
    }

    /**
     * 入金確定処理を実行する。このメソッドで入金確定した場合、入金日は常にSMBCバーチャル口座から通知された入金日となる
     *
     ...
     *                        RESULT_TYPE_10_DISABLED_SMBCPA_ID（無効なSmbcpa ID）
     * @param null | int $oseq 注文SEQ。未確定の場合はnullを指定する
     * @param null | int $accSeq SMBCバーチャル口座SEQ。未確定の場合はnullを指定する
     * @return 入金処理結果コード
     */
     ...
     *
     * @param null | int $oseq 注文SEQ。未確定の場合はnullを指定する
     * @param null | int $accSeq SMBCバーチャル口座SEQ。未確認の場合はnullを指定する
