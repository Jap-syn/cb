 ...
 * @abstract
 */
abstract class LogicHaiponExporterFormatterAbstract implements LogicHaiponExporterFormatterInterface {
    ...
    /**
     * LogicHaiponExporterFormatterAbstractの新しいインスタンスを
     * 初期化する
     */
    public function __construct() {
        $this
            ->setEncoding($this->getDefaultEncoding())
            ->setLineDelimiter($this->getDefaultLineDelimiter());
        $this->_init();
    }
     ...
     * @return LogicHaiponExporterFormatterAbstract このインスタンス
     */
    ...
    public function format(array $data) {
        ...
        foreach($data as $index => $row) {
            $target_row = $this->_fixRow($row);
            $this->_echo($this->_formatRow($target_row));
            $this->_rowFormatted($row, $index);
        }
    ...
    protected function _preFormat(array $data) {
        ...
        $this->_echo($this->_formatRow(array('件数', $dataCount)));
    ...
    /**
     * 1行分の処理完了後の後処理を実行する。
     * このクラスの実装では、クラスに設定されているブロックサイズ分の行を出力するごとに
     * 1行の空白を差し込む処理を行う
     *
     * @access protected
     * @param array $row 出力の元となった行データ。_fixRowを経由しない元データである点に注意
     * @param int $index 行のインデックス位置
     */
    protected function _rowFormatted(array $row, $index) {
        $threshold = self::getBlockSize();
        if((($index + 1) % $threshold) == 0) {
            // ブロックサイズ分出力していたら空行を差し込む
            $target_row = array();
            foreach($this->_fixRow($row) as $col) {
                $target_row[] = '';
            }
            $this->_echo($this->_formatRow($target_row));
        }
    ...
    /**
     * 出力行データを整形し、出力値のみの配列に変換する
     *
     * @access protected
     * @param array $row 出力用の行データ。連想配列
     * @return array 整形済みの値配列
     */
    protected function _fixRow(array $row) {
        return array($row['OrderId'], $row['Deli_JournalNumber']);
    }
