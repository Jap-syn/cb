 ...
 */
class LogicBarcodeDataCvsAtPayment extends LogicBarcodeDataCvsAbstract {
	/**
	...
	public function generateString() {
		return array(
			sprintf('(%s) %s%s-%s%06d%010d%s',
					$this->getAiCode(),         // 2
					$this->getCountryCode(),    // 1
					$this->getMakerCode(),      // 5
					$this->getCorporateCode(),  // 5
					$this->getSiteId(),         // 6
					$this->getUniqueSequence(), // 10
	...
	protected function _generateRawValue() {
		$buf = array(
				...
				$this->getCountryCode(),                                                        // 1
				$this->getMakerCode(),                                                          // 5
				str_pad($this->getCorporateCode(),5,'0'),                                       // 5
				sprintf('%06s', $this->getSiteId()),                                            // 6
				sprintf('%010s', $this->getUniqueSequence()),                                   // 10
				$this->getReIssueCount(),                                                       // 1
                $this->getLimitDate(),                                                          // 6
				$this->getPaymentMoney() >= $this->getStampFlagThresholdPrice() ? '1' : '0',    // 1
