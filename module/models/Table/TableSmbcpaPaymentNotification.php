...
/**
 * T_SmbcpaPaymentNotification(SMBCバーチャル口座入金通知管理)テーブルへのアダプタ
 */
class TableSmbcpaPaymentNotification {
    ...
    protected $_name = 'T_SmbcpaPaymentNotification';
    ...
    {
        $sql  = " SELECT * FROM T_SmbcpaPaymentNotification WHERE NotificationSeq = :NotificationSeq ";
    ...
    public function findByTransactionId($tranId, array $target_status = array(self::STATUS_RECEIVED, self::STATUS_RESPONSED, self::STATUS_NOT_AVAILABLE)) {
        ...
        $sql = " SELECT * FROM T_SmbcpaPaymentNotification WHERE TransactionId = CAST(:TransactionId AS CHAR) AND IFNULL(DeleteFlg, 0) = 0 ";
    ...
    public function findByStatus($status, $limit = 1000) {
        $sql = " SELECT * FROM T_SmbcpaPaymentNotification WHERE Status = :Status AND IFNULL(DeleteFlg, 0) = 0 ORDER BY NotificationSeq ";
        if ($limit >= 1) {
            $sql .= (" LIMIT " . $limit);
        }
    ...
    public function countByStatus($status) {
        $q = <<<EOQ
SELECT COUNT(*) AS cnt
FROM T_SmbcpaPaymentNotification
WHERE
    Status = :Status AND
...
EOQ;
    ...
    public function deletePendingNotifications($oseq, $accSeq, $reason = null) {
        ...
        $sql = " SELECT * FROM T_SmbcpaPaymentNotification WHERE OrderSeq = :OrderSeq AND AccountSeq = :AccountSeq AND IFNULL(DeleteFlg, 0) = 0 AND Status = :Status ";
...
SELECT
    ...
    sum.NameKj
FROM
    T_SmbcpaPaymentNotification nf LEFT OUTER JOIN
...
EOQ;
    ...
    public function countByDate($date) {
        ...
        $sql = " SELECT COUNT(*) AS cnt FROM T_SmbcpaPaymentNotification WHERE ReceivedDate >= :ReceivedDate AND ReceivedDate < (:ReceivedDate + INTERVAL 1 DAY) ";
...
EOQ;
     ...
     * @return プライマリキーのバリュー
     * @see protected function LogicSmbcpaNotificationHandler::createTransactionRow、が呼出しの参考箇所です(20150609_1830)
     */
    public function saveNew($data)
    {
        $sql  = " INSERT INTO T_SmbcpaPaymentNotification (TransactionId, ReceivedDate, Status, ResponseDate, ReceivedRawData, ResponseRawData, ReqBranchCode, ReqAccountNumber, AccountSeq, OrderSeq, ReceiptAmount, ReceiptProcessDate, ReceiptDate, LastProcessDate, RejectReason, DeleteFlg) VALUES (";
        $sql .= "   :TransactionId ";
        $sql .= " , :ReceivedDate ";
        $sql .= " , :Status ";
        $sql .= " , :ResponseDate ";
    ...
    public function saveUpdate($data, $notificationSeq)
    {
        ...
        $sql  = " UPDATE T_SmbcpaPaymentNotification ";
        $sql .= " SET ";
