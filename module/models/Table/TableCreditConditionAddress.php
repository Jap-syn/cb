...
class TableCreditConditionAddress
{
	...
	{
        $sql = " SELECT * FROM T_CreditConditionAddress ORDER BY Seq " . ($asc ? "asc" : "desc");
	...
	{
        $prm = array();
        $sql  = " SELECT * FROM T_CreditConditionAddress WHERE 1 = 1 ";
	...
	{
        $prm = array();
        $sql  = " SELECT COUNT(*) AS Cnt FROM T_CreditConditionAddress WHERE 1 = 1 ";
        foreach ($conditionArray as $key => $value) {
            if ($value == null){
                $sql .= (" AND " . $key . " IS NULL");
            } else {
	...
	{
        $sql = "SELECT Class, COUNT(*) AS Cnt FROM T_CreditConditionAddress WHERE Category = 1 AND ValidFlg = 1 AND Cstring = :Cstring GROUP BY Class ";
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 4 AND Cstring like :domain ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 4 ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring, Point FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 6 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionAddress WHERE ValidFlg = 1 AND Category = 3 AND Cstring like :itemName ";
	...
	{
        $sql  = " INSERT INTO T_CreditConditionAddress (OrderSeq, Category, Class, Cstring, CstringHash, RegistDate, ValidFlg, Point, RegCstring, Comment, RegCstringHash, ComboHash, CreditCriterionId, JintecManualReqFlg, EnterpriseId, SearchPattern, AddConditionCount, RegistId, UpdateDate, UpdateId) VALUES (";
        $sql .= "   :OrderSeq ";
        $sql .= " , :Category ";
        $sql .= " , :Class ";
        $sql .= " , :Cstring ";
	...
	{
        $sql = " SELECT * FROM T_CreditConditionAddress WHERE Seq = :Seq ";
        ...
        $sql  = " UPDATE T_CreditConditionAddress ";
        $sql .= " SET ";
        $sql .= "     Seq = :Seq ";
    ...
    public function getDuplicatedConditionsByComboHash($category, $combo_hash, $enterpriseId, $seq = null) {
        ...
        $sql = " SELECT * FROM T_CreditConditionAddress WHERE " . $where . " ORDER BY Seq DESC ";
	...
	{
	    $prm = array();
	    $sql = " SELECT * FROM T_CreditConditionAddress WHERE ( OrderSeq = -1 ";
 ...
 /**
	...
	public function saveFromArray(array $data) {
        ...
        return $this->_adapter->query(" SELECT * FROM T_CreditConditionAddress WHERE Seq = :Seq ")->execute(array(':Seq' => $pkval));
	}
