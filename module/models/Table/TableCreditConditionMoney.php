...
class TableCreditConditionMoney
{
	...
	{
        $sql = " SELECT * FROM T_CreditConditionMoney ORDER BY Seq " . ($asc ? "asc" : "desc");
	...
	{
        $prm = array();
        $sql  = " SELECT * FROM T_CreditConditionMoney WHERE 1 = 1 ";
	...
	{
        $prm = array();
        $sql  = " SELECT COUNT(*) AS Cnt FROM T_CreditConditionMoney WHERE 1 = 1 ";
        foreach ($conditionArray as $key => $value) {
            if ($value == null){
                $sql .= (" AND " . $key . " IS NULL");
            } else {
	...
	{
        $sql = "SELECT Class, COUNT(*) AS Cnt FROM T_CreditConditionMoney WHERE Category = 1 AND ValidFlg = 1 AND Cstring = :Cstring GROUP BY Class ";
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 4 AND Cstring like :domain ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 4 ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring, Point FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 6 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionMoney WHERE ValidFlg = 1 AND Category = 3 AND Cstring like :itemName ";
	...
	{
        $sql  = " INSERT INTO T_CreditConditionMoney (OrderSeq, Category, Class, Cstring, CstringHash, RegistDate, ValidFlg, Point, RegCstring, Comment, RegCstringHash, ComboHash, CreditCriterionId, JintecManualReqFlg, EnterpriseId, SearchPattern, AddConditionCount, RegistId, UpdateDate, UpdateId) VALUES (";
        $sql .= "   :OrderSeq ";
        $sql .= " , :Category ";
        $sql .= " , :Class ";
        $sql .= " , :Cstring ";
	...
	{
        $sql = " SELECT * FROM T_CreditConditionMoney WHERE Seq = :Seq ";
        ...
        $sql  = " UPDATE T_CreditConditionMoney ";
        $sql .= " SET ";
        $sql .= "     Seq = :Seq ";
    ...
    public function getDuplicatedConditionsByComboHash($category, $combo_hash, $enterpriseId, $seq = null) {
        ...
        $sql = " SELECT * FROM T_CreditConditionMoney WHERE " . $where . " ORDER BY Seq DESC ";
	...
	{
	    $prm = array();
	    $sql = " SELECT * FROM T_CreditConditionMoney WHERE ( OrderSeq = -1 ";
 ...
 /**
     ...
     * @return ResultInterface
     */
    public function judge($category, $regtarget, $conditionSeq)
    {
        $matchWhere = "";
    	...
    	$addWhere = sprintf("ValidFlg = 1 and Category = %s and OrderSeq = -1 %s", $category, $matchWhere);
        ...
        return $this->_adapter->query($query)->execute();
    }
    ...
    public function judgeskip($regtarget, $date = NULL)
    {
        ...
        $addWhere = sprintf("ValidFlg = 1 and Category = 1 and OrderSeq = -1 and Point <= -2000 and %s", $matchWhere);
	...
	public function saveFromArray(array $data) {
        ...
        return $this->_adapter->query(" SELECT * FROM T_CreditConditionMoney WHERE Seq = :Seq ")->execute(array(':Seq' => $pkval));
	}
