...
class TableCreditConditionName
{
	...
	{
        $sql = " SELECT * FROM T_CreditConditionName ORDER BY Seq " . ($asc ? "asc" : "desc");
	...
	{
        $prm = array();
        $sql  = " SELECT COUNT(*) AS Cnt FROM T_CreditConditionName WHERE 1 = 1 ";
        foreach ($conditionArray as $key => $value) {
            if ($value == null){
                $sql .= (" AND " . $key . " IS NULL");
            } else {
	...
	{
        $sql = "SELECT Class, COUNT(*) AS Cnt FROM T_CreditConditionName WHERE Category = 1 AND ValidFlg = 1 AND Cstring = :Cstring GROUP BY Class ";
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 4 AND Cstring like :domain ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 4 ";
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring, Point FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 1 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT Cstring FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 6 AND OrderSeq = -1 ";
        return $this->_adapter->query($query)->execute(null);
	}
	...
	{
        $query = " SELECT COUNT(Seq) AS Cnt FROM T_CreditConditionName WHERE ValidFlg = 1 AND Category = 3 AND Cstring like :itemName ";
	...
	{
        $sql = " SELECT * FROM T_CreditConditionName WHERE Seq = :Seq ";
    ...
    public function getDuplicatedConditionsByComboHash($category, $combo_hash, $enterpriseId, $seq = null) {
        ...
        $sql = " SELECT * FROM T_CreditConditionName WHERE " . $where . " ORDER BY Seq DESC ";
 ...
 /**
    ...
    public function judgeskip($regtarget, $date = NULL)
    {
        ...
        $addWhere = sprintf("ValidFlg = 1 and Category = 2 and OrderSeq = -1 and Point <= -2000 and %s", $matchWhere);
	...
	public function saveFromArray(array $data) {
        ...
        return $this->_adapter->query(" SELECT * FROM T_CreditConditionName WHERE Seq = :Seq ")->execute(array(':Seq' => $pkval));
	}
