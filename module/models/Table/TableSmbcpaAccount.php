...
/**
 * T_SmbcpaAccount(Smbcpa口座)テーブルへのアダプタ
 */
class TableSmbcpaAccount {
    ...
    protected $_name = 'T_SmbcpaAccount';
    ...
    {
        $sql  = " SELECT * FROM T_SmbcpaAccount WHERE AccountSeq = :AccountSeq ";
    ...
    /**
     * 指定の支店コードと口座番号を持つSmbcpa口座データを取得する
     *
    ...
    public function findAccount($branchCode, $accountNumber) {
        ...
        $sql = " SELECT * FROM T_SmbcpaAccount WHERE BranchCode = CAST(:BranchCode AS CHAR) AND AccountNumber = :AccountNumber ";
        return $this->_adapter->query($sql)->execute(array(':BranchCode' => $branchCode, ':AccountNumber' => $accountNumber))->current();
    }

    /**
     * 指定のSmbcpa口座グループに集約されたSmbcpa口座データを取得する。
     *
...
FROM
    T_SmbcpaAccount acc LEFT OUTER JOIN
    T_SmbcpaAccountUsageHistory his ON (
    ...
    T_SmbcpaPaymentNotification nf ON (
...
EOQ;
    ...
    /**
     * 指定のSmbcpa口座グループに集約されたSmbcpa口座データの件数を取得する
     *
     * @param int $groupId Smbcpa口座グループID
     * @param null | array $targetStatus 検索対象のStatus。無効な値は検索時に除外され、
     *                                   有効な値が1つもない場合は未指定として扱われる
     * @return int 指定条件のSmbcpa口座データの件数
     */
    public function countAccountsByGroupId($groupId, array $targetStatus = array()) {
        ...
        $sql = " SELECT COUNT(*) AS cnt FROM T_SmbcpaAccount WHERE AccountGroupId = :AccountGroupId ";
        if (!empty($statusList)) {
    ...
    /**
     * 指定Smbcpa口座グループ配下に登録されたSmbcpa口座の利用状況統計を取得する。
     * 戻り値は口座ステータスをキーとした連想配列で、値の各要素はstatus、label、countの各キーに
     * それぞれ値を対応させた連想配列となる
     *
     * @param int $groupId Smbcpa口座グループ
     * @return array
     */
...
EOQ;
    ...
    /**
     * 指定Smbcpa契約配下で次の請求に使用できるSmbcpa口座データを取得する
     *
     * @param int $smbcpaId Smbcpa ID
     * @return array | null
     */
    public function fetchNextAccount($smbcpaId) {
        $q = <<<EOQ
SELECT
    acc.AccountSeq
FROM
    T_SmbcpaAccount acc FORCE INDEX (Idx_T_SmbcpaAccount03) INNER JOIN
    T_SmbcpaAccountGroup grp ON grp.AccountGroupId = acc.AccountGroupId
WHERE
...
EOQ;
    ...
    /**
     * 指定OEM配下で次の請求に使用できるSmbcpa口座データを取得する
     *
    ...
    public function fetchNextAccountByOemId($oemId) {
        ...
        $smbcpaData = $smbcpaTable->findByOemId($oemId)->current();
        if(!$smbcpaData) {
            throw new \Exception('invalid SmbcpaId specified.');
        }
        return $this->fetchNextAccount($smbcpaData['SmbcpaId']);
    }

    /**
     * 指定注文に関連付けられている請求中Smbcpa口座データを取得する。
...
EOQ;
    ...
    /**
     * 指定注文に関連付けられているクローズ状態のSmbcpa口座データを取得する。
...
EOQ;
    ...
    /**
     * 指定注文に関連付けられている、空き状態ではないSmbcpa口座データを取得する。
...
EOQ;
    ...
    /**
     * 支店コード・口座番号を指定してSmbcpa口座の全利用履歴を取得する
     *
    ...
    {
        $sql  = " INSERT INTO T_SmbcpaAccount (SmbcpaId, AccountGroupId, RegistDate, BranchCode, AccountNumber, AccountHolder, Status, LastStatusChanged, NumberingDate, EffectiveDate, ModifiedDate, SmbcpaStatus, ExpirationDate, LastReceiptDate, ReleasedDate) VALUES (";
        $sql .= "   :SmbcpaId ";
        $sql .= " , :AccountGroupId ";
        $sql .= " , :RegistDate ";
        $sql .= " , :BranchCode ";
