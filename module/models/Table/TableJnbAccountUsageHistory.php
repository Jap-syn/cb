...
/**
 * T_JnbAccountUsageHistory(JNB口座利用履歴)テーブルへのアダプタ
 */
class TableJnbAccountUsageHistory {
	/** 利用履歴タイプ定数：口座使用 */
	...
	protected $_name = 'T_JnbAccountUsageHistory';
	...
	{
	    $sql  = " SELECT * FROM T_JnbAccountUsageHistory WHERE UsageHistorySeq = :UsageHistorySeq ";
...
EOQ;
	...
	public function findByAccountSeq($accSeq, $orderBy = 'DESC') {
        $sql = " SELECT * FROM T_JnbAccountUsageHistory WHERE AccountSeq = :AccountSeq ORDER BY UsageHistorySeq " . $orderBy;

        return $this->_adapter->query($sql)->execute(array(':AccountSeq' => $accSeq));
	}

	/**
	 * 利用履歴タイプとJNB口座SEQ、関連する注文SEQを指定して
	 * 新しい利用履歴を追加する
	 *
	...
	public function addHistory($type, $accSeq, $oseq, $reason = null, $memo = null) {
        ...
        if(!in_array($type, $types)) {
            throw new \Exception('invalid type specified on Table_JnbAccountUsageHistory');
        }
        ...
        // 当該JNB口座の既存履歴の、MostRecentをすべて0に
        $sql = " UPDATE T_JnbAccountUsageHistory SET MostRecent = 0 WHERE AccountSeq = :AccountSeq ";
	...
	/**
	 * 指定のJNB口座の利用履歴をひとつ前の状態にロールバックする。
	...
	public function rollbackHistory($accSeq) {
        ...
        // 最新履歴を削除(論理削除)
        $sql = " UPDATE T_JnbAccountUsageHistory SET MostRecent = 0, DeleteFlg = 1 WHERE UsageHistorySeq = :UsageHistorySeq ";
        $this->_adapter->query($sql)->execute(array(':UsageHistorySeq' => $recent['UsageHistorySeq']));

        if($prev) {
            // 最新前が存在していたらそちらを最新扱いとして更新
            $sql = " UPDATE T_JnbAccountUsageHistory SET MostRecent = 1 WHERE UsageHistorySeq = :UsageHistorySeq ";
	...
	public function findHistoriesByOrderSeq($oseq) {
        $sql = " SELECT * FROM T_JnbAccountUsageHistory WHERE OrderSeq = :OrderSeq AND IFNULL(DeleteFlg, 0) = 0 ORDER BY UsageHistorySeq ";

        return $this->_adapter->query($sql)->execute(array(':OrderSeq' => $oseq));
	}

	/**
	 * 指定JNB口座に関連付けられた口座利用履歴を取得する
	 *
	 * @param int $accSeq JNB口座SEQ
	 * @return ResultInterface
	 */
	public function findHistoriesByAccountSeq($accSeq) {
        $sql = " SELECT * FROM T_JnbAccountUsageHistory WHERE AccountSeq = :AccountSeq AND IFNULL(DeleteFlg, 0) = 0 ORDER BY UsageHistorySeq ";

        return $this->_adapter->query($sql)->execute(array(':AccountSeq' => $accSeq));
	}

	/**
	 * 指定JNB口座に関連付けられた最新の口座利用履歴を取得する
	 *
	 * @param int $accSeq JNB口座SEQ
	 * @return array | null
	 */
	public function findRecentHistoryByAccountSeq($accSeq) {
        $sql = " SELECT * FROM T_JnbAccountUsageHistory WHERE AccountSeq = :AccountSeq AND MostRecent = 1 AND IFNULL(DeleteFlg, 0) = 0 ORDER BY UsageHistorySeq DESC ";
	...
	/**
	 * 指定JNB口座に関連付けられた、最新のひとつ前の口座利用履歴を取得する
	 *
	 * @param int $accSeq JNB口座SEQ
	 * @return array | null
	 */
	public function findPrevHistoryByAccountSeq($accSeq) {
        $sql = " SELECT * FROM T_JnbAccountUsageHistory WHERE AccountSeq = :AccountSeq AND MostRecent = 0 AND IFNULL(DeleteFlg, 0) = 0 ORDER BY UsageHistorySeq DESC ";
        $row = $this->_adapter->query($sql)->execute(array(':AccountSeq' => $accSeq))->current();
...
EOQ;
	...
	{
        $sql  = " INSERT INTO T_JnbAccountUsageHistory (AccountSeq, UsedDate, MostRecent, Type, OrderSeq, CloseReason, CloseMemo, DeleteFlg) VALUES (";
        $sql .= "   :AccountSeq ";
        $sql .= " , :UsedDate ";
        $sql .= " , :MostRecent ";
        $sql .= " , :Type ";
	...
	public function saveUpdate($data, $usageHistorySeq)
	{
        ...
        $sql  = " UPDATE T_JnbAccountUsageHistory ";
        $sql .= " SET ";
