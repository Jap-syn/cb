...
class Font_Glyph_Outline_Simple extends Font_Glyph_Outline {
  ...
  public function encodePoints($points) {
    ...
    if(!empty($endPtsOfContours)){
        $endPtsCount = count($endPtsOfContours);
    }

    // count関数対策
    $l += $font->writeInt16($endPtsCount); // endPtsOfContours
    ...
    // Simple glyf
    // count関数対策
    $l += $font->w(array(self::uint16, $endPtsCount), $endPtsOfContours); // endPtsOfContours
    ...
    $xCount = 0;
    if(!empty($coords_x)){
        $xCount = count($coords_x);
    }

    // count関数対策
    $l += $font->w(array(self::int16, $xCount), $coords_x); // xCoordinates

    // count関数対策
    $yCount = 0;
    if(!empty($coords_y)){
        $yCount = count($coords_y);
    }

    // count関数対策
    $l += $font->w(array(self::int16, $yCount), $coords_y); // yCoordinates
    return $l;
  } 
