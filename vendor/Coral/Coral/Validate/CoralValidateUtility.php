...
/**
 * 入力値検証を補助するユーティリティクラス
 */
class CoralValidateUtility {
	/**
	 * メールアドレスとの一致をチェックするためのPerl互換正規表現ソース
	 *
	 * @var string
	 */
	const EMAIL_ADDRESS = '/^[^,]+[@＠][^,]+[.．][^,]+$/';
	...
	/**
	 * メールアドレスとの部分一致をチェックするためのPerl互換正規表現ソース
	 * (2011.03.25 追加 eda)
	 *
	 * @var string
	 */
	const EMAIL_ADDRESS_PART = '/^[^,]*[@＠]?[^,]+([.．]?[^,]+)*$/';

	/**
	 * 電話番号との一致をチェックするためのPerl互換正規表現ソース
	 *
	 * @var string
	 */
	const PHONE_NUMBER = '/^[- 0-9－　０-９]+$/';

	/**
	 * 郵便番号との一致をチェックするためのPerl互換正規表現ソース
	 * (2008.02.08 変更 eda)
	 *
	 * @var string
	 */
	const POSTAL_CODE = '/^[0-9０-９]{3}[\-－ー]?[0-9０-９]{4}$/';

	/**
	 * 整数との一致をチェックするためのPerl互換正規表現ソース。
	 * 先頭のマイナス記号を許容し、先頭の数字の0は1桁の場合のみ許容する
	 *
	 * @var string
	 */
	...
	/**
	 * 0以上の整数との一致をチェックするためのPerl互換正規表現ソース。
	 * 先頭のマイナス記号は許容せず、先頭の数字の0は1桁の場合のみ許容する
	 *
	 * @var string
	 */
	const NON_NEGATIVE_INTEGER = '/^(\d|([1-9]\d+))$/';

	/**
	 * 正の整数との一致をチェックするためのPerl互換正規表現ソース。
	 * 先頭のマイナス記号は許容せず、0から始まってはいけない
	 *
	 * @var string
	 */
	const POSITIVE_INTEGER = '/^([1-9]|([1-9]\d+))$/';

	/**
	 * ペイジー連携用メールアドレス正規表現ソース
	 *
	 * @var string
	 */
	const PE_EMAIL_ADDRESS = '/^[a-zA-Z_0-9\-\.]+@[a-zA-Z_0-9\-\.]+$/';
	 ...
	 * @var string
	 */
	const PE_PHONE_NUMBER = '/^[\-0-9]+$/';

	/**
	 * @static
	 *
	 * 指定の文字列を'3桁-4桁'形式の郵便番号表記に変換する。
	 * 入力文字は半角数字のみに変換された後、3桁より多い場合は3桁-残り桁、
	 * 3桁以下の場合はその数字のみに整形される。
	 *
	 * @param string $postalCode 変換する文字列。全角・半角の数字以外は完全に無視される
	 * @param boolean $zeroPrefix 7桁以下の入力に対してゼロプレフィックスを行うかのフラグ。省略時はfalse
	 * @return $postalCodeを3桁-4桁で整形した郵便番号表記の文字列
	 */
	public static function fixPostalCode($postalCode, $zeroPrefix = false) {
		// 半角に整形
		$postalCode = BaseGeneralUtils::convertWideToNarrow( "$postalCode" );
		// 数字以外を除去
		$postalCode = preg_replace('/[^\d]/', '', $postalCode);
		// 指定があったら7桁になるようゼロプレフィックス（2011.6.27 eda, 6.28 suffix -> prefix）
		if($zeroPrefix && strlen($postalCode) < 7) {
			$postalCode = substr('0000000' . $postalCode, -7);
		}
		// 3桁-4桁に整形
   ...
   *
   * @param string $namekj
   * @param boolean $colon 項目名とメッセージをコロンで区切らない場合はfalseを渡す(個別注文修正用)
   * @return エラーがある場合はエラー情報の配列を、エラーがない場合はnullを返す
   */
	public static function checkPeNameKj($namekj, $colon = true){
    $trimVal = str_replace(array(' ','　'), '', $namekj);
	  if(!(0 < mb_strlen($trimVal))){
      if($colon) {
          $sub = "氏名：";
          $msg = "データを空にすることはできません";
      } else {
          $sub = "'氏名'を";
          $msg = "入力してください";
      }
    ...
    }else if(preg_match('/(?:\xEF\xBD[\xA1-\xBF]|\xEF\xBE[\x80-\x9F])|[\x20-\x7F]/', $namekj)){
      $sub = $colon ? "氏名:" : "'氏名'は";
      return array(
              'subject'=> $sub,
              'message' => '全角で入力してください',
    ...
    }else if(!self::checkDoubleByteCharacter($namekj)){
      $sub = $colon ? "氏名:" : "'氏名'に";
      return array(
              'subject'=> $sub,
   ...
   * @param boolean $colon 項目名とメッセージをコロンで区切らない場合はfalseを渡す(個別注文修正用)
	 * @return エラーがある場合はエラー情報の配列を、エラーがない場合はnullを返す
	 */
	public static function checkPeNameKn($namekn, $colon = true){
    $trimVal = str_replace(array(' ','　'), '', $namekn);
    ...
    }else if(!(mb_strlen($namekn) <= 48)){
      $sub = $colon ? "氏名カナ:" : "'氏名カナ'は";
      return array (
              'subject'=> $sub,
              'message' => '48文字以内で入力してください',
              'error_code' => 202
      );
    }else if(!mb_ereg("^[゛゜ァ-ヶー－　（）／．０-９Ａ-Ｚａ-ｚ]+$",$namekn)){
      $sub = $colon ? "氏名カナ:" : "'氏名カナ'は";
      return array(
              'subject'=> $sub,
	...
	/**
	 * ペイジー用にメールアドレスのチェックを行う
	 *
	 * ※メール必須サイトの検証は別途必要
	 * @param string $mailaddress
   * @param boolean $colon 項目名とメッセージをコロンで区切らない場合はfalseを渡す(個別注文修正用)
   * @param boolean $require trueの場合、必須チェックを行う
	 * @return エラーがある場合はエラー情報の配列を、エラーがない場合はnullを返す
	 */
	public static function checkPeMailAddress($mailaddress, $colon = true, $require = false){
	  if(is_null($mailaddress) || mb_strlen($mailaddress) <= 0){
	    if($require){
	          if($colon) {
          $sub = "メールアドレス：";
          $msg = "データを空にすることはできません";
    ...
    else if(!(mb_strlen($mailaddress) <= 128)){
      $sub = $colon ? "メールアドレス:" : "'メールアドレス'は";
      return array(
              'subject'=> $sub,
              'message' => '128文字以内で入力してください',
              'error_code' => 202
      );
    }else if(!preg_match(self::PE_EMAIL_ADDRESS, $mailaddress)){
      $sub = $colon ? "メールアドレス:" : "'メールアドレス'の";
      return array(
              'subject'=> $sub,
              'message' => '入力データが不正です',
              'error_code' => 202
   ...
   * @param boolean $colon 項目名とメッセージをコロンで区切らない場合はfalseを渡す(個別注文修正用)
	 * @return エラーがある場合はエラー情報の配列を、エラーがない場合はnullを返す
	 */
	public static function checkPePhoneNumber($phone, $colon = true){
    $trimVal = str_replace(array(' ','　'), '', $phone);
	  if(!(0 < mb_strlen($trimVal))){
      if($colon) {
          $sub = "電話番号：";
          $msg = "データを空にすることはできません";
      ...
      $sub = $colon ? "電話番号:" : "'電話番号'は";
      return array(
              'subject'=> $sub,
              'message' => '13文字以内で入力してください',
              'error_code' => 202
      );
	  }else if(!preg_match(self::PE_PHONE_NUMBER, $phone)){
      $sub = $colon ? "電話番号:" : "'電話番号'の";
      return array(
              'subject'=> $sub,
              'message' => '入力データが不正です',
              'error_code' => 202
	 ...
	 * 全角用正規表現チェック
	 */
	protected static function checkDoubleByteCharacter($targetText){
