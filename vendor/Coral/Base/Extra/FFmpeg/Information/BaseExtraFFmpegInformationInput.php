 ...
 * @class
 *
 * 入力ファイル情報を解析するNetB_Extra_FFmpeg_Information_Interface実装クラス
 */
class BaseExtraFFmpegInformationInput implements BaseExtraFFmpegInformationInterface {
	/**
	 * @static
	 * @protected
	 *
	 * 入力ファイル情報の開始行に一致するPerl互換正規表現のパターン
	 *
	 * @var string
	 */
	protected static $__firstLine = '/^Input #(.*), ([^,]+),([^,]+,)* from (.*):$/';

	/**
	 * @static
	 * @protected
	 *
	 * 入力ファイル情報の各行を解析する正規表現パターンの配列
	 *
	 * @var array
	 */
	 ...
	 * @return bool $lineが入力ファイル情報の開始行の場合はtrue、それ以外はfalse
	 */
	...
	protected static function _getExpressions() {
		if( self::$__expressions == null ) {
			self::$__expressions = array(
				'summary' => '/  Duration: ([^,]+), start: ([^,]+), bitrate: (.+)$/',
				'stream' => '/    Stream #([^:]+): ([^:]+): (.+)$/'
			);
		}
	 ...
	 * @constructor
	 *
	 * 入力ファイル情報開始行を指定して、NetB_Extra_FFmpeg_Information_Inputの
	 * 新しいインスタンスを初期化する
	 *
	...
	public function __construct($line) {
		...
		$this->_path = str_replace( "'", '', "$matches[4]" );
	}
	...
	public function parseLine($line) {
		foreach( self::_getExpressions() as $key => $exp ) {
			...
			case 'stream':
				...
				switch( $kind ) {
				case 'video':
					$map = split( '/', 'format/pixel format/size/bitrate/frame rate' );
					break;
				case 'audio':
					$map = split( '/', 'format/sampling rate/channels/bitrate' );
					break;
				}
				if( $map ) {
					$info = array();
	...
	/**
	 * 入力ファイルのメディアフォーマットを取得する。
	 * 戻り値は'mpeg'、'avi'、'mp3'などになる。
	 *
	 * @return string
	 */
	...
	/**
	 * 再生時間やメディアビットレートなどの一般情報を格納する
	 * 連想配列を取得する。
	 * 戻り値のキーは'duration'（再生時間）や'bitrate'などがある。
	 *
	 * @return array
	 */
