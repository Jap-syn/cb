    ...
    ResultInterface
{
    ...
    protected function loadDataFromMysqliStatement()
    {
        ...
        if ($this->statementBindValues['keys'] === null) {
            ...
            // count関数対策
            $count = 0;
            if (!empty($this->statementBindValues['keys'])){
                $count = count($this->statementBindValues['keys']);
            }

            $this->statementBindValues['values'] = array_fill(0, $count, null);
            $refs = array();
       ...
       $statementCount = 0;
       if (!empty($this->statementBindValues['keys'])){
           $statementCount = count($this->statementBindValues['keys']);
       }

        // dereference
        for ($i = 0, $count = $statementCount; $i < $count; $i++) {
