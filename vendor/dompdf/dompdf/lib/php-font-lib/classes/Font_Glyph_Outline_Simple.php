...
class Font_Glyph_Outline_Simple extends Font_Glyph_Outline {
    ...
    while($i < $l) {
      switch($path[$i]) {
        ...
        case "z":
          ...
          $points[$pointsCount-1]["endOfContour"] = true;
  ...
  public function encodePoints($points) {
    ...
    $font = $this->getFont();
    // count関数対策
    $endPtsOfContoursCount = 0;
    if (!empty($endPtsOfContours)){
      $endPtsOfContoursCount = count($endPtsOfContours);
    }
    $l = 0;
    $l += $font->writeInt16($endPtsOfContoursCount); // endPtsOfContours
    ...
    // Simple glyf
    // count関数対策
    $l += $font->w(array(self::uint16, $endPtsOfContoursCount), $endPtsOfContours); // endPtsOfContours
    ...
    $l += $font->w(array(self::uint8, $flagsCount), $flags); // flags
    // count関数対策
    $coords_xCount = 0;
    if (!empty($coords_x)){
      $coords_xCount = count($coords_x);
    }
    $l += $font->w(array(self::int16, $coords_xCount), $coords_x); // xCoordinates
    // count関数対策
    $coords_yCount = 0;
    if (!empty($coords_y)){
      $coords_yCount = count($coords_y);
    }
    $l += $font->w(array(self::int16, $coords_yCount), $coords_y); // yCoordinates
    return $l;
  } 
