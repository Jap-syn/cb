  ...
  protected function o_pages($id, $action, $options = '') {
        ...
        if (isset($options['id']) && isset($options['rid']) && isset($options['pos'])) {
          ...
          if (isset($o['info']['pages'][$i]) && $o['info']['pages'][$i] == $options['rid']) {
            ...
            if ($k >= 0) {
              // count関数対策
              $infoPageCount = 0;
              if (!empty($o['info']['pages'])){
                $infoPageCount = count($o['info']['pages']);
              }
              for ($j = $infoPageCount-1 ; $j >= $k; $j--) {
    ...
    case 'out':
      ...
      if (!empty($o['info']['pages'])) {
        ...
        // count関数対策
        if ( (isset($o['info']['fonts']) && (!empty($o['info']['fonts']))) ||
              isset($o['info']['procset']) ||
              (isset($o['info']['extGStates']) && !empty($o['info']['extGStates']))) {
          $res.= "\n/Resources <<";
          ...
          // count関数対策
          if (isset($o['info']['xObjects']) && !empty($o['info']['xObjects'])) {
            $res.= "\n/XObject << ";
          ...
          // count関数対策
          if ( isset($o['info']['extGStates']) && !empty($o['info']['extGStates'])) {
            $res.= "\n/ExtGState << ";
    ...
    switch ($action) {
      ...
      // count関数対策
        if (!empty($o['info']['outlines'])) {
          ...
          // count関数対策
          $outlinesCount = count($o['info']['outlines']);
          $res.= "] /Count ".$outlinesCount ." >>\nendobj";
        } else {
...
//      }
  ...
  function utf8toCodePointsArray(&$text) {
    ...
    for ($i = 0; $i < $length; $i++) {
      ...
      if ($bytesCount === 0) { // get starting octect
        if ($c <= 0x7F) {
    ...
    if ( $current_font['isUnicode']) {
      ...
      if ( $char_spacing != 0 ) {
        // count関数対策
				$unicodeCount = 0;
				if (!empty($unicode)){
					$unicodeCount = count($unicode);
				}
        $w += $char_spacing * $space_scale * ($unicodeCount + $n_spaces);
      }
