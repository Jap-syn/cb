...
class DataMatrix {
	...
	*/
	function draw_by_size($code, $size)
	{
		...
		$img_output =ImageCreate($w_base*$size, $h_base*$size);
...
class DxImageEncoderOptions {
	...
	function DxImageEncoderOptions () 
	{
		//$white = ImageColorAllocate($img, 0xFF, 0xFF, 0xFF);
		//$black = ImageColorAllocate($img, 0x00, 0x00, 0x00);
		$white = 0xFFFFFF;
...
class DxRegion {
	...
	function DxRegion ($src) // [DxRegion src]
	{
		$this->SizeIdx = DxCodeSize::DxSzRectAuto;
...
class DxCommon {
	public function genReedSolEcc ($message, $sizeIdx) // [DxMessage message, DxCodeSize sizeIdx]
	{
		$g = array();
		$b = array();
...
class DxMessage {
	...
	public $Code;	// byte[]
	public $Output;	// byte[]
	public function DxMessage ($sizeIdx, $symbolFormat) // [DxCodeSize sizeIdx, DxFormat symbolFormat]
	{
...
class DxChannel {
	...
	public function DxChannel() { // default class members
...
class DxChannelGroup {
	...
	public function DxChannelGroup () 
	{
...
class DxMatrix3 {
	...
	public function DxMatrix3 ($src) // [DxMatrix3 src]
	{
		$this->__init();
		if($src != null)
		{
...
class DxImage {
	...
	public function DxImage ($pxl, $width, $height, $pack) // [byte[] pxl, int width, int height, DxPackOrder pack]
	{
		$this->__init();
