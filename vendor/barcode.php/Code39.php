<?php
	 ...
	 * バーコードの描画を行います。バーコード全体の幅を指定するのではなく、バーを描画する横方向の最小単位のドット数を指定します。(1～)
	 * @param $code 描画を行うバーコードのコード(テキスト)
	 * @param $width 横方向の最少描画ドット数
	 * @param $height バーコードのバーの高さ(単位：ドット)
	 * @return バーコードのイメージを返します。
	 */
//	function Draw($code, $width, $height) {
//
//		global $TextWrite, $FontName, $FontSize, $dispStartStopCode;
//
//		$code = "*".strtoupper($code)."*";
//
//		$x0 = 1;
...
//
//				if ($j % 2 == 0) {
//					for ($k = 0; $k < $dot[$x0or1]; $k++) {
//						imageline($img, $xPos+$k, 1, $xPos+$k, $height, $black);
//					}
//				}
//				$xPos += $dot[$x0or1];
//			}
...
//		}
//
//		//$width = $xPos-60; bitma は？
//		$output_image = ImageCreate($width, $gazouHeight);
//		//ImageCopyResampled($output_image,$img,0,0,0,0,$width,$gazouHeight,$xPos,$gazouHeight);
//		ImageCopyResized($output_image,$img,0,0,0,0,$width,$gazouHeight,$xPos,$gazouHeight);
//
//
//		//添字の描画
//		if($this->TextWrite) {
//			if($this->dispStartStopCode) {
//				$interval = ($width - $this->FontSize) / (strlen($code) - 1);
//				for($i = 0; $i < strlen($code); $i++) {
//					ImageTTFText($output_image, $this->FontSize, 0, $i * $interval, $gazouHeight
//						,$black, $this->FontName, substr($code, $i, 1));
//				}
//			} else {
//				$interval = ($width - $this->FontSize) / (strlen($code) - 2 - 1);
//				for($i = 1; $i < strlen($code) - 1 ; $i++) {
//					ImageTTFText($output_image, $this->FontSize, 0, $i * $interval, $gazouHeight
//						,$black, $this->FontName, substr($code, $i, 1));
//				}
